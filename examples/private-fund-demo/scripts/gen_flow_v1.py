...
依赖 .claude/skills/flowchart/scripts/gen_flow_base.py
"""
import sys
...
# 向上查找 .claude/skills/flowchart/scripts
_here = Path(__file__).resolve().parent
for _p in [_here, *_here.parents]:
    _target = _p / ".claude/skills/flowchart/scripts"
    if _target.exists():
        sys.path.insert(0, str(_target))
        break
else:
    raise RuntimeError("flowchart skill scripts not found in ancestors")
