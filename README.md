...
![用工程化纪律替代 prompt 玄学：会议纪要 → 场景清单 → 交互大图 → 原型 → PRD](assets/readme/hero.png)
...
| data-report | 周报 / 月报 / 季报，行为分析平台 + 有数自动化 |
...
| 内部项目系统 | 内部项目管理 CLI 桥接（任务 / 需求 / 进度查询），「工具型 Skill 怎么包内部系统」的范例 |
...
│   ├── fetch_confluence.py
│   ├── fetch_figma.py
│   ├── pull_meeting_notes.py    #   会议纪要工具纪要拉取
│   ├── md_to_confluence.py
