...
用法：
    python3 scripts/analyze_gate_funnel.py                          # 默认回看 30 天
    python3 scripts/analyze_gate_funnel.py --days 7
    python3 scripts/analyze_gate_funnel.py --gate context-static-lint

前置：读 .claude/logs/usage.jsonl（工区遥测日志；缺文件时报空结果不报错），
     无 .env / 凭证依赖，--help 本身不需要。
