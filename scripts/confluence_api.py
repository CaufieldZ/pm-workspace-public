...
_r and (_s.path.insert(0, str(_r / "scripts")), __import__("lib.route_log", fromlist=["emit"]).emit("confluence_api"))
...
def main(argv: list[str] | None = None) -> int:
  ...
  # CQL 搜页面取 ID 列表
  python3 confluence_api.py '/rest/api/content/search?cql=type=page+and+space=DEMO_SPACE_KEY&limit=5' --jq '.results[].id'
  # 带 body 的 POST（-f 值支持 JSON 字面量）
  python3 confluence_api.py /rest/api/content -X POST -f 'type=page' -f 'space.key=DEMO_SPACE_KEY' -f 'title=测试'
  # 从文件读 body（- 读 stdin）
