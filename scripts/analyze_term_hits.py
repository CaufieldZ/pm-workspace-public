...
用法：
    python3 scripts/analyze_term_hits.py
    python3 scripts/analyze_term_hits.py --days 90
    python3 scripts/analyze_term_hits.py --gate plain-language-gate --min-scans 100

前置：本地事件日志 .claude/logs/usage.jsonl（近 N 天事件；--help 本身不需要日志存在）；
运行时子进程调 scripts/dump_term_inventory.py 取词表快照。无 .env / 凭证依赖。

退出码：恒 0（分析类，结论走 stdout，无阻断语义）。
"""
