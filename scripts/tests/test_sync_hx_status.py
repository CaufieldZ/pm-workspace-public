"""sync_hx_status 纯函数回归：内部项目系统字段解析 / 状态映射 / PRD TAG 日期 / 两条管线回写。

只测纯函数（吃字符串 → 返回结构），不触网络——内部项目系统 调用在 sync_one 的 I/O 层，
纯函数与网络解耦，这里锁定「内部项目系统状态 → 本地缓存投影」的映射与回写契约。
"""
...
def test_parse_hx_work_id_real():
    assert s.parse_hx_work_id("- **内部项目系统**：https://INTERNAL_URL_REDACTED<ID>）") == "402040"


def test_parse_hx_work_id_bare_h():
    assert s.parse_hx_work_id("**内部项目系统**：H<ID>") == "388202"


def test_parse_hx_work_id_placeholder_none():
    assert s.parse_hx_work_id("- **内部项目系统**：https://INTERNAL_URL_REDACTED<work_id>（H 号）") is None
...
| :--- | :--- | :--- | :--- |
| PRD 版本 | 2.3 | 状态 | 开发中 |
| 拟制人 / 日期 | 维护者 / 2026-07-25 | 内部项目系统 | https://INTERNAL_URL_REDACTED<ID>） |
