...
# 3. 红人模块
"""
TERMINAL_MODULES = """# 1. 抽奖模块
状态：已结束（活动 2026-06-30 下线）
最后核对线上：2026-01-01
...
def test_parse_module_freshness_skips_terminal_states():
    by_title = {t: (d, raw) for t, d, raw in parse_module_freshness(TERMINAL_MODULES)}
    # 终态模块（campaign 冻结）过期日期也不报——线上状态不再演化；无日期也不占位
    assert by_title == {"社区模块": (date(2026, 6, 1), "2026-06-01")}
...
def test_delta_in_flight_excludes_merged_and_archived():
    # 第四层作用域：已合并 / 已归档的历史 delta 不该再被要求补内部项目系统链接
...
def test_delta_in_flight_keeps_registered_but_unmerged():
    # changelog 有行但状态未推进到「已合并」→ 仍在途，内部项目系统链接照样该补
    archive = Path("p/deliverables/archive")
