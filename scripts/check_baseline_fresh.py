...
第二层 · 内容新鲜度（机械验不了，黄灯不卡）：
  ...
  按模块章精确报，不是整份一个日期。未上线（无线上可核对）与终态
  （已结束 / 已冻结 / 已下线——campaign 冻结）模块章跳过。
...
第三层 · 合并痕迹（机械可算，红灯）：
  ...
  判据是**子串匹配**：delta 实体核心词（如「连麦角色状态机」）须在 baseline 对应
  小节标题里原样出现，措辞偏离（写成「连麦嘉宾状态」）即便内容已搬也报红灯——
  反向合并时 baseline 小节标题照抄 delta 实体核心词。
...
前置：无 .env / 凭证依赖，纯本地扫描（--help 本身不需要任何输入）。输入 =
projects/{产品线}/prd-*-baseline.md + deliverables/（含 archive/）下的 delta PRD。
可选环境变量 SKIP_MERGE_TRACE=1 跳过第三层合并痕迹校验。
...
# delta 头部「内部项目系统」字段（工作项链接，产研效能上线日期/状态唯一权威源）
_HX_FIELD = re.compile(r"^[\s\-*]*\**\s*内部项目系统\s*\**\s*[:：]\s*(.+?)\s*$", re.MULTILINE)
...
def delta_in_flight(delta_path: Path, changelog: dict[str, str], archive_dir: Path) -> bool:
    ...
    第四层（内部项目系统链接）只该管在途的 —— 给已合并 / 已归档的历史 delta 补内部项目系统链接没有
...
def delta_hx_link_missing(delta_text: str) -> bool:
    """delta 头部无「内部项目系统」字段 / 字段为占位符（含 <work_id>）→ True。"""
...
# 未上线 / 终态模块不参与线上核对，第二层跳过：
#   未上线 / 待排期 / 规划中 —— 无线上可核对
#   已结束 / 已冻结 / 已下线 —— 活动 / 模块生命周期终了，线上状态不再演化（campaign 基线冻结用）
_NOT_LIVE = re.compile(r"状态[\*）)]*\s*[:：]?\s*\**\s*(未上线|待排期|规划中|已结束|已冻结|已下线)")
...
def parse_module_freshness(baseline_text: str) -> list[tuple[str, date | None, str]]:
    ...
    标注「状态：未上线 / 待排期 / 规划中 / 已结束 / 已冻结 / 已下线」的模块章跳过
    （未上线无线上可核对；终态模块线上状态不再演化）。
    """
...
def check(project: str, repo_root: Path, stale_days: int) -> int:
    ...
    # ── 第四层：delta 必绑内部项目系统链接（离线可验证，warn 不拦）──
    stale: list[str] = []
    hx_missing: list[str] = []
    ...
    for dp in delta_files:
        ...
        if delta_in_flight(dp, changelog, archive_dir) and delta_hx_link_missing(text):
            hx_missing.append(f"  · {dp.name} 头部无「内部项目系统」字段或为占位符——跑 sync_hx_status.py 前先补内部项目系统工作项链接")
    ...
    if hx_missing:
        print(f"\n🟡 第四层 · 内部项目系统链接缺失（{len(hx_missing)}，提醒不卡）——内部项目系统是上线日期/状态唯一权威源：")
        for s in hx_missing:
            print(s)
    else:
        print("🟢 第四层 · 内部项目系统链接：所有 delta 均已绑定内部项目系统工作项")
...
def main() -> int:
    ...
    ap.add_argument("project", help="项目 / 产品线路径片段（projects/ 下的目录名，见 projects/ 下目录名；必填）")
