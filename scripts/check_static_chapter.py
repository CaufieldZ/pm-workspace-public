...
用法：
    ...
    python3 scripts/check_static_chapter.py projects/<产品线>/scene-list.md --json-out -

参数：
    <file>...            baseline md / scene-list.md（可多个；hook 只喂这两类）
    --json-out <path|->  命中明细写 JSON（- 走 stdout；hook 取命中词埋点用）

前置：无（纯本地 md 扫描；--help 本身不需要）。

退出码：
    0 — 扫描完成（全部命中走 warn 于 stderr，hook 不阻断）
    1 — 未传文件（用法错误）
"""
...
def main() -> None:
    ...
    if not files:
        print("未传要扫的文件。用法见 python3 scripts/check_static_chapter.py --help", file=sys.stderr)
        sys.exit(1)
