...
前置：无 .env / 凭证依赖（--help 本身不需要任何输入）。扫描对象 = 本文件内
FORK_GROUPS 登记的源头/副本路径，时间信号取 git log 与文件 mtime（无 git 历史
的未跟踪文件只报「内容不同」不判方向）。

退出码：恒 0 —— 输出是待人工判断的清单，不是 fail。
"""
...
def main() -> int:
    ...
    ap.add_argument("--diff", action="store_true", help="打印每组 diff 摘要（默认关）")
    ap.add_argument("--group", help="只查一组（取值：FORK_GROUPS 组名，如 youshu_cli / check_cjk_punct；默认全查）")
    args = ap.parse_args()
