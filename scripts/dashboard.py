#!/usr/bin/env python3
"""Workspace Dashboard — 聚合 .claude/logs/usage.jsonl + projects/ 状态，生成工作区看板。

看板含项目视图 / Skill 使用 / hub 产线 / Sub-agent 调度 / 快捷路由热度 /
Hook 健康度 / 异常告警（--days ≥ 28 加 Gate 周 block 率趋势）。

用法：
    python3 scripts/dashboard.py                  # 默认统计过去 14 天
    python3 scripts/dashboard.py --days 30        # 自定义窗口（≥28 追加 Gate 趋势章）

前置：无（usage.jsonl 缺失时按空事件渲染；--help 本身不需要）。
产物：.claude/workspace-dashboard.md（覆盖写；session-start hook 自动注入项目视图节）。
退出码：0 生成成功（无其他失败分支）。
"""
import argparse
import json
...
def dir_mtime(parent: Path, extra: list[Path]) -> float:
    """取项目关键文件（baseline / scene-list / deliverables 内容）的最大 mtime。

    deliverables 取**目录内文件**的最大 mtime，不取目录自身——目录 mtime 只在直接子项
    增删时变，改 {季度}/{版本}/ 里的产物不动它，「最近活动」会一直显示成旧日期。
    点文件（.DS_Store 等）不算活动。
    """
...
def render_routes(events, days):
    ...
    # CLAUDE.md 快捷路由表里**模型主动调**的脚本 name（埋点 name 与脚本绑定，dashboard 侧固定枚举）
    # 新加快捷路由脚本时三处同步：脚本头埋点 + 本列表 + CLAUDE.md 表，否则本表看不到它
    ...
    KNOWN_ROUTES = [
        ...
        "slack", "market_cli", "dashboard", "sync_pools",
...
def main():
    ...
    parser.add_argument("--days", type=int, default=14,
                        help="统计窗口天数（默认 14；≥ 28 时追加 Gate 周 block 率趋势章）")
