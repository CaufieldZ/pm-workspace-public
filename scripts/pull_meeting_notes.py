...
前置：dingtalk-doc MCP 配置（~/.claude.json 本项目节，或仓库根 .mcp-disabled.json）
且本地 MCP HTTP 服务可用；--help 本身不需要。
产物：projects/{项目}/inputs/meetings/meeting-YYYY-MM-DD.md（同日多场加主题 slug 后缀）。
退出码：1 = 项目目录不存在 / MCP 配置缺失 / 调用失败（消息见 stderr）。
"""
