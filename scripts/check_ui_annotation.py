...
Usage:
    python3 scripts/check_ui_annotation.py projects/<产品线>/deliverables/<季度>/<版本>/proto-<产品线>-<版本>-<端>.html
    python3 scripts/check_ui_annotation.py projects/<产品线>/archive/base/deliverables/imap-<产品线>-v<N>.html --strict

参数：
    <html>...  proto-*.html / imap-*.html（按文件名路由：proto- 走原型屏，
               imap- / 含 interaction 走交互大图屏；其他文件名跳过不扫）
    --strict   有违规 exit 2（hook 阻断用）

前置：无（纯本地 HTML 扫描；--help 本身不需要）。
