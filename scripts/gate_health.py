...
前置：无 .env / 凭证依赖，纯本地扫描（--help 本身不需要日志存在）。输入 =
.claude/logs/usage.jsonl 遥测（缺失 → skip）+ .claude/hooks/*.sh / .githooks/ /
scripts/*.sh 的 gate 注册表 + scripts/dashboard.py 的 GHOST_GATES。
...
def main() -> int:
    ...
    ap.add_argument("--strict", action="store_true", help="有红灯时 exit 2（默认关，warn 不阻断）")
