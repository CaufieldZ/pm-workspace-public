#!/bin/bash
# audit-fast.sh — PostToolUse 快速自检（< 1s）：对刚写入的 deliverable 做秒级拦截
set -uo pipefail

usage() {
  cat <<'EOF'
PostToolUse 快速自检（< 1s）：检查 deliverable 产物的场景编号一致性 / 命名前缀规范 / 占位符残留 / 字体顺序等。
用法：
    bash scripts/audit-fast.sh <file>
参数：
    <file>    产物文件路径；仅检查 .../projects/<项目名>/deliverables/ 下的 .html/.md/.docx，其余路径静默放行
示例：
    bash scripts/audit-fast.sh <仓库>/projects/<项目名>/deliverables/<file>.html
前置：无（--help 不需要）
退出码：
    0 = 通过或不在检查范围（静默）
    1 = 违规（stderr 一行说明）
...
# 提取项目名。用 `(.+)` 而非 `([^/]+)`：项目可嵌一层（projects/growth/queen/），
# 单段式匹配不到时会原样输出整条路径 → SCENE_LIST 指向不存在的文件 → 场景检查静默跳过。
PROJECT=$(echo "$FILE" | sed -E 's|.*/projects/(.+)/deliverables/.*|\1|')
SCENE_LIST="$ROOT/projects/$PROJECT/scene-list.md"
# 未替换时 PROJECT 仍是绝对路径（sed 原样输出）→ 显式报警，不再静默
case "$PROJECT" in
  /*) echo "⚠️ audit-fast: 无法从路径提取项目名，场景编号检查跳过：$FILE" >&2 ;;
esac
