...
### `--help` 五要素标准（三层用法分工的兜底层）

脚本用法三层分工：CLAUDE.md 快捷路由（入口）→ cli-cheatsheet（多子命令 / 复杂参数）→ 脚本 `--help`（兜底）。`--help` 输出必须让调用方（多数是模型）不看源码答五件事：

| # | 要素 | 要求 |
|---|------|------|
| ① | 一句话职责 | 干什么 + 何时用；= docstring 首行（README 提取源） |
| ② | 可抄示例 2-4 条 | 路径 / 项目名一律占位符形态（`<产品线>` / `<项目>` / `<版本>` / `<file>`；日期用 YYYY-MM-DD 格式示例）；**禁真实项目名、真实交付物路径、内部域名、内部业务术语**——help 文案随公开镜像直出（`.sh` 不脱敏）；checker 类至少 1 条最简 + 1 条 `--stdin` 管道形态；生成类写明产物落点模式 |
| ③ | 参数 help | 每个 `add_argument` 带 `help=`，写清取值格式与「默认 X」（argparse 不自动展示 default）；手写 / bash 版在用法段逐参数一行 |
| ④ | 前置 | `.env` 变量 / 凭证 / 本地输入依赖，注明 `--help` 本身不需要；无前置写「前置：无」 |
| ⑤ | 退出码语义 | 检查类必写（照上表）；生成类写产物落点 |

实现形态按脚本现状选：

- **argparse**：`ArgumentParser(description=__doc__, epilog=<示例块>, formatter_class=argparse.RawDescriptionHelpFormatter)`——docstring 按 §一 模板含五要素，单源不双写；子命令型每个子命令 `help=` + epilog 每子命令一条示例
- **手写 `sys.argv` 解析**：入口最前 `if "-h" in args or "--help" in args: print(__doc__); return 0`，分支必须在任何真实执行（扫描 / 网络 / 写文件）之前
- **bash**：头部 `case "${1:-}" in -h|--help) usage; exit 0 ;; esac`；`usage()` 用 `<<'EOF'` heredoc 防变量展开，末尾显式 `exit 0`

help 文案措辞与 cli-cheatsheet 已覆盖的脚本保持同名同义（不搬内容，避免双份维护点）；禁沿革叙述（「原来 / 改成」），只写现在时事实。
...
| `stat` | BSD `-f '%Sm'`，GNU `-c '%y'` | BSD 优先 `\|\|` GNU fallback；写法见 § I 分叉约定 |
...
CLI 缺失走 `command -v x >/dev/null 2>&1` 守卫静默降级（如 macOS 专属 `open` / `pbcopy`），不让缺命令把脚本拖崩。
...
- [ ] `--help` 覆盖五要素（§二：职责 / 示例 / 参数 help / 前置 / 退出码），且 `--help` 本身零副作用
