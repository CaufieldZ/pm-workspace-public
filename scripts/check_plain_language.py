...
Usage:
    ...
    python3 scripts/check_plain_language.py <file>... --strict --added-only
...
def _parse_added(diff_text: str) -> set[int]:
    """从 `git diff -U0` 输出解析新增行号集合（纯函数，便于单测）。"""
    added: set[int] = set()
    for line in diff_text.splitlines():
        m = _HUNK_RE.match(line)
        if m:
            start = int(m.group(1))
            added.update(range(start, start + int(m.group(2) or 1)))
    return added


def _added_lines(path: Path) -> set[int] | None:
    """该文件相对 HEAD 的新增行号集合；None = 不做过滤。

    None 的三种情形：不在此仓库 / 未跟踪的新文件 / git 调用失败。新文件没有存量违规
    可豁免，按全量扫；git 出问题一律降级为全量，不因环境问题让门静默放行。
    """
    try:
        r = subprocess.run(["git", "ls-files", "--error-unmatch", str(path)],
                           capture_output=True, text=True)
        if r.returncode != 0:
            return None  # 未跟踪 → 新文件，全量扫
        r = subprocess.run(["git", "diff", "-U0", "HEAD", "--", str(path)],
                           capture_output=True, text=True)
        if r.returncode != 0:
            return None
    except (OSError, subprocess.SubprocessError):
        return None
    return _parse_added(r.stdout)


def check_file(path: Path, added_only: bool = False) -> tuple[list, list]:
    """返回 (strict_hits, warn_hits) 两个列表。

    added_only：只报该文件相对 HEAD 的新增行（存量违规多的 living 文档不因旧账被拦）。
    """
    if is_skipped(path):
        return [], []
    if is_exempted(path):
    ...
    if added_only:
        ...
        if added is not None:
            strict_hits = [h for h in strict_hits if h[0] in added]
            warn_hits = [h for h in warn_hits if h[0] in added]
...
def main():
    ...
    if not files:
        print("用法: check_plain_language.py <file>... [--strict] [--stdin] "
              "[--added-only] [--json-out <path|->]", file=sys.stderr)
        sys.exit(0)
    ...
    for f in files:
        if not f.exists():
            continue
        strict_hits, warn_hits = check_file(f, added_only=added_only)
