...
子命令：
  servers                      列出所有已配置 server（名 / 传输 / 地址）
  list <server>                列该 server 的全部工具（名 + 描述前 60 字）
  call <server> <tool> [参数]  调用工具；参数 = 单个 JSON 串，或 key=val 多个
                               （value 先按 JSON 解析，失败则当字符串）

前置：无凭证要求（server 配置里自带）；--help / servers 之外的真实调用按
各 server 配置联网。--help 本身不需要任何 .env。

退出码：0 成功；1 未知 server / 未知子命令 / 参数解析失败 / 网络 or MCP 错误
（错误原因写 stderr，server 不可用时附 fallback 建议）。
"""
...
_r and (_s.path.insert(0, str(_r / "scripts")), __import__("lib.route_log", fromlist=["emit"]).emit("call_mcp"))
