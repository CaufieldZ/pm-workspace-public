...
前置：无鉴权（公开 REST）；依赖 Python requests 包。--help 本身不需要网络。
...
_r and (_s.path.insert(0, str(_r / "scripts")), __import__("lib.route_log", fromlist=["emit"]).emit("market_cli"))
...
def main() -> int:
    ...
    p.add_argument("-q", "--quote", default=DEFAULT_QUOTE, help="计价币后缀 usdt/btc/.../all（默认 usdt）")
    p.add_argument("--top", type=int, default=15, help="取前 N（默认 15）")

    p = sub.add_parser("kline", parents=[common], help="K线")
    p.add_argument("symbol", help="单币对，如 BTC / BTC-USDT / btcusdt（裸 base 自动补 usdt）")
