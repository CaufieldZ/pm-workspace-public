#!/bin/bash
# impact-check.sh — 场景影响检测：scene-list 变更后比对 deliverables/，找出需更新的过期产物
set -euo pipefail

usage() {
  cat <<'EOF'
场景影响检测：比对 projects/<项目名>/scene-list.md 与 deliverables/ 下产物引用的场景编号，找出场景变更后需要更新的交付物。
用法：
    bash scripts/impact-check.sh <项目名>
参数：
    <项目名>    projects/ 下的项目目录名（英文短名）
示例：
    bash scripts/impact-check.sh <项目名>
前置：在 git 仓库内运行；<项目名> 需含 scene-list.md 与 deliverables/（--help 不需要）
退出码：
    0 = 全部同步（含仅提示级差异）
    1 = 有产物需要更新，或项目目录 / scene-list.md / deliverables/ 不存在
EOF
}
case "${1:-}" in
  -h|--help) usage; exit 0 ;;
