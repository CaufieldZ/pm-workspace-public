...
| 脚本 | 快捷路由 | 职责 |
|------|---------|------|
| `call_mcp.py` | 联网搜索（无 WebSearch 模型必跑 / 中文站点优先） / 网页读取（已知 URL 抓正文） / 读开源仓库/GitHub 源码 / Outlook 邮件/日历/联系人 | 通用 MCP 调用脚本——HTTP 和 stdio 均支持，不需要在 Claude Code 里加载 server。 |
| `check_baseline_fresh.py` | 现状一致性（baseline 新鲜度 / 同源副本漂移） | baseline PRD 新鲜度校验（文档集模型承重不变量的 definition-of-done）。 |
...
| `check_fork_drift.py` | 现状一致性（baseline 新鲜度 / 同源副本漂移） | 同源脚本副本漂移登记（源头改了、脱敏副本没跟上的可见化）。 |
| `check_generator_docstring.py` | 规则层合规（LEARNED / 体积棘轮 / 版本漂移 / 生成脚本 docstring） | 项目生成脚本 docstring 自证充分性检查（轻量 lint）。 |
| `check_learned_rules.py` | 规则层合规（LEARNED / 体积棘轮 / 版本漂移 / 生成脚本 docstring） | LEARNED.md 已沉淀规则的硬阻断检查器（按文件类型分发）。 |
| `check_rule_version_drift.py` | 规则层合规（LEARNED / 体积棘轮 / 版本漂移 / 生成脚本 docstring） | 规则版本 drift 校验 — 验产物声明的骨架版本是否落后于当前规则版本。 |
| `check_rule_volume.py` | 规则层合规（LEARNED / 体积棘轮 / 版本漂移 / 生成脚本 docstring） | 规则层体积棘轮 —— 逐文件上限，只降不升，升须论证。 |
| `check_staged_large_files.py` | git 提交前体检 | git staged 文件体检 — pre-commit 阻断超大文件 + project 本地源素材入库（warn / block 两级，阈值走 env var）。 |
| `check_static_chapter.py` | 静态章写作纪律（baseline 模块章/现状章/scene-list 静态章四不） | 真相源静态章写作纪律 lint（baseline 模块章 / 现状章 / scene-list）。 |
...
| `nav_confluence.py` | 不知道 pageId/URL 定位页面 / 内容词全文搜索（哪页提到 X / 哪个 space 有） | Confluence 页面导航：不知道 pageId/URL 时按标题 / 内容词定位页面、按父页看子页树。 |
...
| `check_code_review_gate.py` | — | pre-commit 强制 code review 门槛 — 重大代码变更（infra 桶或总 churn 超阈值）未经 review 拦截提交。 |
