...
用法：
    python3 scripts/check_learned_rules.py projects/<产品线>/scene-list.md
    python3 scripts/check_learned_rules.py projects/<产品线>/deliverables/<季度>/<版本>/prd-<产品线>-<版本>.md
    python3 scripts/check_learned_rules.py projects/<产品线>/scene-list.md --warn-only

前置：无（纯本地 md 扫描；--help 本身不需要任何输入）。文件不存在 → 直接通过。

退出码：
    0 — clean / warn（无违规，或仅 PRD snake_case warn，或 scene-list 违规但传了 --warn-only）
    2 — scene-list View 前缀违规且未传 --warn-only（hook 阻断用）
"""
...
def main() -> int:
    ...
    parser.add_argument(
        "file", type=Path,
        help="要检查的文件路径（按文件名路由：scene-list*.md 查 View 前缀，prd-*.md 查裸 "
             "snake_case，其他文件名跳过不报）")
    parser.add_argument(
        "--warn-only",
        action="store_true",
        help="即使有 strict 命中也只 warn 不 exit 2（默认严格模式，即默认可阻断）",
    )
