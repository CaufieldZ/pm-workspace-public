...
# 单次批量拉取硬顶（安全限制）：search_pages / fetch_children / fetch_attachments(download)
# 的单次运行上限，超限截断 + stderr 告警；CLI 显式超参（如 dig --limit）在入口直接报错。
BATCH_PULL_CAP = 50
...
def search_pages(cql: str, limit: int = 10, expand: str | None = None) -> list:
    _ensure_creds()
    if limit > BATCH_PULL_CAP:
        print(f"警告：limit={limit} 超单次批量拉取上限 {BATCH_PULL_CAP}，已截断（安全限制）", file=sys.stderr)
        limit = BATCH_PULL_CAP
...
def search_content(cql: str, limit: int = 10) -> list:
    """内容词全文搜索：走 /rest/api/search（带高亮摘要，excerpt 含 @@@hl@@@ 标记）。

    与 search_pages（/rest/api/content/search，纯列表无摘要）互补：想知道「哪个 space 的哪页
    含内容词」用本函数。结果字段：title / excerpt（HTML 实体 + @@@hl@@@ 标记）/ url（相对）/
    resultGlobalContainer（space 名 + /display/<key> 地址）/ content（dict，含 type）。
    """
    _ensure_creds()
    if limit > BATCH_PULL_CAP:
        print(f"警告：limit={limit} 超单次批量拉取上限 {BATCH_PULL_CAP}，已截断（安全限制）", file=sys.stderr)
        limit = BATCH_PULL_CAP
    encoded = urllib.parse.quote(cql)
    data = api_request("GET", f"/rest/api/search?cql={encoded}&limit={limit}")
    return data.get("results", [])
...
def fetch_attachments(page_id: str, download: bool = False, filter_names: set | None = None) -> dict:
    ...
    download=True 单次最多下 BATCH_PULL_CAP 张，超限截断 + 告警（安全限制）。
    """
    LIMIT = 50
    mapping: dict = {}
    downloaded = 0
    ...
    while True:
        ...
        for att in results:
            ...
            if download:
                if downloaded >= BATCH_PULL_CAP:
                    print(f"警告：该页图片达单次批量拉取上限 {BATCH_PULL_CAP}，其余截断未下（安全限制）", file=sys.stderr)
                    return mapping
...
def fetch_children(parent_id: str, limit: int = 50) -> list:
    ...
    单次最多 BATCH_PULL_CAP 个子页，超限截断 + 告警（安全限制）。
    """
    children = []
    start = 0
    while True:
        ...
        results = res.get("results", [])
        children.extend(results)
        if len(children) >= BATCH_PULL_CAP:
            print(f"警告：子页数达单次批量拉取上限 {BATCH_PULL_CAP}，更多子页不再拉取（安全限制）", file=sys.stderr)
            del children[BATCH_PULL_CAP:]
            break
        if len(results) < limit:
            break
        start += limit
