...
前置：无 .env / 凭证依赖，纯本地扫描（--help 本身不需要 hub/ 存在；无 hub/ 时正式
运行直接 skip）。第三层对照 hub/deployed.json 的人工登记（缺失则该层降级为
「全部未登记」）。
...
def main() -> int:
    ...
    ap.add_argument("--strict", action="store_true", help="有红灯（zip 过期）时 exit 2（默认关，warn 不阻断）")
    args = ap.parse_args()
