...
前置：无 .env / 凭证依赖，纯本地扫描（--help 本身不需要任何输入）。版本基准 =
.claude/skills/_shared/workspace.json 的 skel_version（未配 / 格式非 vN → skip）；
输入 = projects/{产品线}/ 下的 baseline md 与 deliverables/（含 archive/）产物。
...
def main() -> int:
    ...
    ap.add_argument("project", nargs="?",
                    help="项目 / 产品线路径片段（projects/ 下的目录名，见 projects/ 下目录名；"
                         "传 --rule-sources 时可省略，否则必填）")
    ap.add_argument("--rule-sources", action="store_true",
                    help="输出规则源清单（文件 + 目录），供 post-commit hook 取用（默认关）")
    args = ap.parse_args()
