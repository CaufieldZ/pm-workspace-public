...
usage() {
  cat <<'EOF'
按需启用 / 禁用 MCP server，省上下文 token。
用法：
    bash scripts/toggle-mcp.sh [status]                       查看所有 server 状态（默认）
    bash scripts/toggle-mcp.sh on <server> [<server> ...]     启用指定 server
    bash scripts/toggle-mcp.sh off <server> [<server> ...]    禁用指定 server
    bash scripts/toggle-mcp.sh only <server> [<server> ...]   只保留指定，其余全禁用
参数：
    status    查看启用 / 禁用清单与各自 token 估算（无参数时默认动作）
    on        启用列出的 server
    off       禁用列出的 server
    only      只保留列出的 server，其余全禁用
示例：
    bash scripts/toggle-mcp.sh status
    bash scripts/toggle-mcp.sh off <server1> <server2>
    bash scripts/toggle-mcp.sh only <server>
前置：无（--help 不需要）；改动写入 .mcp.json / .mcp-disabled.json / ~/.claude.json，改完需重启 Claude Code 生效
退出码：
    0 = 成功（含 status 查看）
    1 = 未知操作，或 on / off / only 未带 server 参数
EOF
}
case "${1:-}" in
  -h|--help) usage; exit 0 ;;
