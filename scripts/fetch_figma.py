...
凭证读取优先级（--help 本身不需要）:
  1. 环境变量 FIGMA_PAT（或 FIGMA_API_KEY；.env 会自动 source）
  2. .mcp-disabled.json → mcpServers.figma.env.FIGMA_PAT
  3. .mcp.json → mcpServers.figma.env.FIGMA_PAT

产物落点：--image 默认 stdout（-o 文件 / -p 则 projects/{项目}/inputs/figma/）；
--batch 默认 figma-export/（-p / --out-dir 可改）。结构摘要 / --nodes / --tree / --search 走 stdout。
退出码：1 = 无凭证 / URL 不识别 / 文件无权限等（消息见 stderr）。
"""
...
_r and (_s.path.insert(0, str(_r / "scripts")), __import__("lib.route_log", fromlist=["emit"]).emit("fetch_figma"))
import argparse
import json
import os
import re
...
def load_pat():
    ...
    for src in [ROOT / ".mcp-disabled.json", ROOT / ".mcp.json"]:
        if src.exists():
            ...
            env = expand_refs(cfg.get("mcpServers", {}).get("figma", {}).get("env", {}))
...
def parse_batch_arg(batch_arg):
    ...
    return _parse_json_nodes(p.read_text(encoding="utf-8"))
...
def main():
    ...
    parser.add_argument("-p", "--project", help="项目名，截图存到 projects/{项目}/inputs/figma/")
    ...
    if args.batch:
        ...
        if not items:
            sys.exit("错误：批量列表为空")
        if args.project:
            out_dir = _resolve_project_dir(args.project) / "inputs" / "figma"
        elif args.out_dir:
