...
产物：语料 md——批量模式 dig-{关键词}.md、单页模式 dig-page-{pageId}.md；落点 -p 时
projects/{项目}/inputs/docs/，--out-dir 时该目录，都不传则当前目录（--images 时图落 assets/）。
退出码：1 = 参数互斥 / 命中 0 篇等（sys.exit 消息体见 stderr）。
"""
...
from lib.confluence import BATCH_PULL_CAP, api_request, base_url, fetch_attachments, get_page, search_pages  # noqa: E402
...
def main():
    ...
    ap.add_argument("--space", default="DEMO_SPACE_KEY", help="Confluence space key（默认 DEMO_SPACE_KEY）")
    ...
    ap.add_argument("--limit", type=int, default=25, help=f"最多拉几篇（默认 25，取最新 N 篇；单次上限 {BATCH_PULL_CAP}）")
