#!/usr/bin/env python3
"""内部项目系统 → 本地 delta 状态/上线日期同步（内部项目系统是唯一权威源，本地是缓存投影）。

从 delta 头部「内部项目系统」字段解析 H 号 → 读内部项目系统真实流程状态 + PRD TAG 上线日期
→ 回写本地两条管线：
  管线 1（反向合并门）：delta 头部「状态」字段
  管线 2（周报上线归因）：delta §排期 / 上线节奏表 含「上线/灰度/全量」行的 起止 + 状态

默认 dry-run（print diff）；--apply 才落盘。本地「已合并」是本地动作（内部项目系统无此态），不被覆盖。
...
前置：内部项目系统凭证 AIHUB_TOKEN（通常 source .env；或对单次传 --token）+ skill 内置 内部项目系统 二进制；
--help 本身不需要。
退出码：0 正常（含 dry-run / 无改动）；1 文件不存在 / 参数缺失；2 认证失败。
上线日期口径：内部项目系统 PRD TAG 打标时间（repo tag history --prefix PRD 最早一条）。
...
_r and (_s.path.insert(0, str(_r / "scripts")), __import__("lib.route_log", fromlist=["emit"]).emit("sync_hx_status"))
...
# 复用 skill 的 内部项目系统 封装
_SKILL_SCRIPTS = find_root() / ".claude" / "skills" / "内部项目系统" / "scripts"
sys.path.insert(0, str(_SKILL_SCRIPTS))
import hx_client as hx  # noqa: E402

CONTROLLED = ("待排期", "开发中", "已上线", "已合并")

# 内部项目系统 current_process → 本地受控词。已合并是本地动作，内部项目系统无此态，此表不产出「已合并」。
...
_HX_CELL = re.compile(r"\|\s*内部项目系统\s*\|\s*([^|\n]*?)\s*\|")
...
def header_span(delta_text: str) -> tuple[int, int]:
    ...
    协作表的「状态」「内部项目系统」格只在此区间内认，避免撞上 §排期表的「状态」列头。
    """
...
def parse_hx_work_id(delta_text: str) -> str | None:
    """从头部「内部项目系统」字段抽 work_id（bullet 或协作表两种形态）。占位符返回 None。"""
...
def map_status(current_process: str, has_prd_tag: bool) -> str:
    """内部项目系统流程 + 是否有 PRD TAG → 本地受控词。未知节点返回空串（保留本地值）。"""
...
def sync_one(path: Path, token: str | None, apply: bool) -> dict:
    ...
    if not work_id:
        rec["notes"].append("❌ 头部无「内部项目系统」字段或为占位符（未填 H 号）——先补内部项目系统链接再同步")
        return rec
    rec["work_id"] = work_id
    ...
    if not new_status:
        rec["notes"].append(f"内部项目系统流程「{proc}」未在映射表内 → 保留本地状态，未改（请人工确认映射）")
    else:
...
def render(rec: dict) -> str:
    ...
    if rec.get("work_id"):
        ...
        lines.append(f"  内部项目系统：[{rec.get('work_name') or ''}]({rec.get('link')})"
