...
Usage:
    python3 scripts/learned_stats.py                       # 统计根目录 LEARNED.md
    python3 scripts/learned_stats.py --learned other.md    # 指定别的教训文件

前置：无（规则覆盖判断只 grep 本仓库规则文件；--help 本身不需要）。
输出形态：stdout 三档统计（总条目 / 已覆盖 / 待归位 / 重复主题组），不写文件。
退出码：0 正常；1 --learned 指向的文件不存在。
"""
