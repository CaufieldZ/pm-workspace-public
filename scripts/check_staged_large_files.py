...
用法：
    python3 scripts/check_staged_large_files.py    # 检查当前 git staged 文件（无参数，pre-commit hook 自动调）

前置：无（读 git index）。阈值 env var：STAGED_FILE_WARN_BYTES（默认 500000）/ STAGED_FILE_BLOCK_BYTES（默认 2000000）。

退出码：
    0 — 放行（warn 级只打印不阻断）
    1 — 有 block 级违规（超大文件 / project 本地源素材），pre-commit 阻断
"""
...
def is_local_project_artifact(path: str) -> bool:
    ...
    if ext in LOCAL_ONLY_EXTS and ("/inputs/" in normalized or "/deliverables/" in normalized):
        # moderation 词库表 xlsx = 内容安全服务/客服交接物本体，入库（与 .gitignore 豁免同口径）
