...
用法：
    ...
    python3 scripts/check_delta_conflict.py <产品线>

前置：无 .env / 凭证依赖，纯本地扫描（--help 本身不需要任何输入）。输入 =
projects/{产品线}/prd-*-baseline.md 的变更记录表 + deliverables/ 下的在途 delta。
SKIP_DELTA_CONFLICT=1 整体跳过。

退出码：恒 0（warn 级，供 hook / dashboard 调）。结构化结论走 stdout。
非 baseline 项目 / 在途 delta < 2 → skip。
"""
