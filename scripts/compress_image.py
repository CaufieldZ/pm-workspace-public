...
用法（hook 阻断模式）：
    python3 scripts/compress_image.py img.png --strict

前置：Python Pillow 库（PIL）；--help 本身不需要。
产物：默认同目录 {stem}-compressed{suffix}（不覆盖原图）；--inplace 才原地覆盖。
...
_r and (_s.path.insert(0, str(_r / "scripts")), __import__("lib.route_log", fromlist=["emit"]).emit("compress_image"))
