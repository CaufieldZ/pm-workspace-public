...
> v27: 2026-04-28 Confluence 推送 pageId 缓存：push_to_confluence_base.py 自动读写 `projects/{项目}/.confluence.json`（含 prd_page_id / space / parent_id / title），首次推送写入，后续零参数推送，新 session 不再问 pageId；多 PRD 项目用 pages map 按 docx 文件名匹配（直播间 V2.8/V3.0 自动分发）；--space 默认 jituankejizhongxin；删除遗留 scripts/confluence-pages.json（无人读取）；回填 5 个活跃项目配置（htx-activity-center / htx-community / htx-community-leaderboard / htx-live-streaming-updateQ2 / LT-HTX-Liquidity）
