...
【端能力校验】
产出物中画的每个交互（按钮/跳转/弹窗），必须确认目标端有能力执行：
- H5 页面不能画需要 TRTC/WebRTC 的功能（如开播、推流、设备检测）
- OBS 主播视角不能画 TRTC 专属功能（如开播准备弹窗、画面源切换）
- App 原生端不能画需要浏览器才能运行的功能
- 如果某个按钮点了之后要跳到另一个端才能完成，用箭头 + 文案标注「→ 见 D-0d」，不要在当前端画完成态
...
产 md 的 skill（scene-list / bspec / pspec / test-cases / data-report / competitor-analysis / workspace-audit）交付后，用户问「同步 Confluence / 推到 wiki」时，走 `scripts/md_to_confluence.py`，不要用 Confluence MCP 的 `create_page` 直接传 `bodyStorageValue`（38KB 的 md 当参数会吃 10K+ input token）。

脚本用法：

```bash
# 建新页（parent-id 从 Confluence URL pageId 取）
python3 scripts/md_to_confluence.py <md路径> --parent-id <父页面id>

# 覆盖已有页
python3 scripts/md_to_confluence.py <md路径> --update-id <页面id>
```

默认空间 `jituankejizhongxin`，标题取 md 一级标题，creds 从 `.mcp.json` 读。md 会自动包 Confluence Markdown 宏（前提是目标空间装了该宏，HTX wiki 已装）。
...
**自检**：编号与 scene-list 一致 + 脚本存 scripts/ 命名 `gen_{类型}_v{N}.py`（PPT 用 `.js`）且成对交付 + 无 `FILL_START/FILL_END/待填充` 残留 + 术语全文一致。
