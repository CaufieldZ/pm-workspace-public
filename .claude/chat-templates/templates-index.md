...
> **覆盖产物：** 方案架构图集 / 交互大图 / 可交互原型 / PRD
>
> **【ChatOpus vs 本地 Opus 分工】**
> - ChatOpus 仅做决策辩论 / 审计 / 方案 review，不写产出物
> - context.md / scene-list / IMAP / PRD 等所有产出物由本地 Opus 写
> - ChatOpus 给出的决策建议由本地 Opus 落地到 context.md 对应章节
...
├─ 从0-1需求？
│   ├── 用户 描述需求背景
│   ├── ChatOpus 和 用户 讨论方案、对齐场景、角色、核心决策（不写 context.md）
│   ├── 本地 Opus 把讨论落地写入 context.md → 用户 确认
│   └── 进入 0.2 产物选择
│
├─ 已有项目迭代？
│   ├── 用户 上传历史 context.md
│   ├── ChatOpus 读取，提供决策建议 / 方案 review
│   ├── 用户 说明本轮变更点 → 本地 Opus 落地写入 context.md → 用户 确认
│   └── 进入 0.2 产物选择
│
└─ 用户 直接指定产物？
...
| **术语一致** | context.md 术语表是唯一真相源，交互大图/原型/PRD 全局统一 |
| **编号对齐** | scene-list 编号 = 交互大图 Scene 编号 = 原型页面编号 = PRD 章节内 Scene 编号 |
| **主线一致性** | context.md 第 4 章场景叙事顺序 = IMAP 各 PART `part-story` = PRD 第 2 章主线图。三处都从 context 第 4 章顶部那句叙事顺序长出来 |
...
## 5 侧边栏演示文稿（sidebar-deck-template.html）
