...
pg_scripts_first() {
  ...
  echo "   → 阈值依据: scripts/lib/thresholds.yaml write_tool.html_max_lines（200 行）" >&2
...
PY
  ...
  # guide 文件本身缺席（如 gitignored 的 hub L2 公司规范在全新 clone / 同事工区不存在）→
  # 不可读，强行要求会 fail-closed 死锁（无满足路径，只能 SKIP env 绕，等于废掉整条 hub 产线）。
  # 降级：缺席 guide 记 ABSENT、不进 MISSING；存在的 guide 仍严格必读（不削弱真·required-read）。
  local MISSING=() ABSENT=()
  local _g
  for _g in "$GUIDE" "$GUIDE2" "$GUIDE3" "$GUIDE4"; do
    [ -z "$_g" ] && continue
    if [ ! -f "$PROJECT_DIR/$_g" ]; then ABSENT+=("$_g"); continue; fi
    _pg_check_read "$_g" || MISSING+=("$_g")
  done
