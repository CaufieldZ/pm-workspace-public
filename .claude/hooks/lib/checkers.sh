...
check_cjk_for_bash_recent() {
  ...
  local recent
  recent=$(find_recent_deliverables 30 '*.html' '*.md' '*.drawio' '*.mmd')
  [ -z "$recent" ] && return 0
...
check_plain_language_for_bash_recent() {
  ...
  while IFS= read -r f; do
    ...
    local tmpout
    tmpout=$(mktemp)
    # --added-only：只拦本次新增行。living 文档（baseline / 老 delta）存量违规多，
    # 全量扫会让每次编辑都被旧账拦下，只能 SKIP 绕过、门沦为噪音。手动复查不传该 flag。
    python3 "$checker" "$f" --strict --added-only > "$tmpout" 2>&1
    local rc=$?
    if [ "$rc" -eq 2 ]; then
      fail=1
