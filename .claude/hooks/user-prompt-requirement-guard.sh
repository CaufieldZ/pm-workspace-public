...
#   3. 无指标语境数字（Q3 / v2 / A-1 等顺带数字不算量化目标）
...
# ── 条件 3：无指标语境数字（无量化目标）───────────────────────────────────
# 「含任意数字」不足以证明已给量化目标——Q3 / v2 / A-1 / P0 / 9月 这类顺带数字
# （季度 / 版本 / 场景号 / 优先级 / 排期）不是指标。先剥掉这些非指标数字 token，
# 剩余仍含数字才算真给了量化目标 → 跳过提醒；剥完无数字 → 视为模糊需求。
...
printf '%s' "$PROMPT_METRIC" | grep -qE '[0-9]' && exit 0
