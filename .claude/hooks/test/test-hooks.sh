...
echo ""
echo "── user-prompt-requirement-guard.sh · 量化目标判定（F1：Q3/v2 顺带数字不算指标）──"
# 提醒走 stdout systemMessage；dedup 600s 全局键 stale 即 touch 占配额 → 每个 fire 断言前清缓存
_pmg_clear() { rm -f "${TMPDIR:-/tmp}/pmws_dedup/pm-gate-reminder."* 2>/dev/null; }
run_reqguard() { printf '{"prompt":"%s"}' "$1" | bash .claude/hooks/user-prompt-requirement-guard.sh 2>/dev/null; }
_pmg_clear
