...
# 快速放行：≤ 500 行直接过。500 = thresholds.yaml read_gate.large_file_lines 的快路径同值
# （改该键须同步此 500 与下方 fallback 500）。绝大多数文件命中此分支，省每次 awk 解析 yaml。
[ "$LINE_COUNT" -le 500 ] && exit 0

# 仅 > 500 行才精确读阈值（yaml 真相源，可能被调高于 500）。awk 失败 fallback 500。
THRESHOLD=$(awk '/^read_gate:/{f=1; next} f && /^[a-z]/{exit} f && /large_file_lines:/{gsub(/[^0-9]/,"",$2); print $2; exit}' "${CLAUDE_PROJECT_DIR:-$(pwd)}/scripts/lib/thresholds.yaml" 2>/dev/null)
