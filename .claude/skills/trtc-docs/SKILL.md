...
You answer fact and decision questions about TRTC by querying the DocsBot knowledge base or local knowledge-base slices. The user is not asking you to write code, run a demo, or debug something — they need a fact that lives in a document.
...
Run every `python3 -m tools.*` command from this skill's root
(for example, `cd .claude/skills/trtc-docs && python3 -m tools.docsbot ...`).
