...
**B. 按 View / 页面 / 模块拆分**：每个 View 或页面独立一份文档，各自自包含。适用于多设计师 AI 并行工作。

**默认推荐规则**（向用户推荐时依此）：

- 场景数 ≥ 20 **或** View ≥ 3 → 默认推 B（按 View 拆分）。理由：设计/前端 AI 上下文消费压力，单份 ≤ 800 行更可用
...
| 优先级 | 节数 | 每节体量 | 说明 |
|--------|------|---------|------|
| P0 | 4 节全写（模块 / 状态矩阵 / 交互流 / 异常） | 模块 ≥ 5 个、状态矩阵 ≥ 3 列 | 核心页面，详细展开 |
| P1 | 4 节全写 | 模块 3-5 个、状态矩阵 2-3 列 | 次要页面，写骨架不堆细节 |
| P2 | 合并 2 节：「核心模块 + 关键异常」 | 模块 ≤ 3 个 | 低频页面，一段话够 |
| 后续迭代 / 占位 | 不展开 | 1 段话说明原因 | 如 D-4 结算报告 V3 做，只标注「Q2 仅基础实现 + V3 规划」 |
| 后端服务场景（无 UI） | 跳过 | 不写入 pspec | 如 G-1 返佣归因，pspec 天然跳过 |

### Step 2.6：场景组合并（强制）

识别条件（满足任一即合并）：

- 共享同一底栏/壳子的连续编号场景（如 A-1 默认态 / B-3 在麦态 / B-4a 拉黑态 / B-4b 锁麦态 共用 A-1 壳子，变化只在底栏按钮启用/置灰）
- 共享同一容器/范式的场景（如 A-2 / C-1c / C-2 / C-3 共用「底部半屏浮层」范式）

合并写法：

- 合成 1 个「Scene Group」节，标题写 `Scene B-3 ~ B-4b：在麦底栏状态组`
- 模块拆解章节写共性部分，差异用**状态矩阵表**展开（每个子状态一列）
- 每个子状态只写与共性的差异，不重复描述底栏结构

反例：A-1 底栏 M7 在 B-3 / B-4 / B-4a / B-4b 的模块拆解中重复描述 → 改为 Scene Group 的共性模块 + 状态矩阵列差异。
...
- [ ] **单份行数 ≤ 800**：超出则必须按 View 拆分或精简 P2/占位场景，防止设计/前端 AI 上下文撑爆
- [ ] **场景详略分档**：P0 详写、P1 精简、P2 合并 2 节、后续迭代不展开、后端无 UI 场景跳过
- [ ] **场景组合并**：共享底栏/容器的连续场景已合并为 Scene Group（如 A-1+B-3+B-4+B-4a+B-4b 底栏状态组）
...
LINES=$(wc -l < "$FILE")
echo "行数: $LINES"
[ "$LINES" -gt 800 ] && echo "❌ 超出 800 行上限，请按 View 拆分或精简 P2/占位场景（设计/前端 AI 上下文消费压力）" || echo "✅ 行数可控"
