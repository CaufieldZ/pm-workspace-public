...
- `assets/fill-template.js` / `gen-notes-docx.py` / `presenter-mode.js` — 脚本，通过 node/python3 调用
...
const used = [
  'page-hero','hero-headline','hero-sub','hero-accent',
  'page-split','split-num','split-title','split-desc',
  'stat-card','stat-val','stat-lbl',
  'grid2','grid3','grid4',
  'flow-h','flow-h-step','fh-num','fh-label','fh-desc',
  'pipe','pipe-node','pipe-arrow','pipe-icon',
  'cmp-table','quote-block',
  'eyebrow','eyebrow-accent','hairline','hairline-sm','hairline-accent',
  'display','display-xl','display-lg','display-md',
  'lede','pullquote','pullquote-cite','figure-num','figure-lbl',
  'watermark-tl','film-grain','section-label',
];
...
**不要发明新类名**。`ppt-template.html` 是唯一类名来源。
...
| 流程图页 | flowchart skill 独立产出 → 截图嵌入 PPT | 审批流、决策树、分支判断、泳道图 |
| 架构图页 | 手画 platform-card 三段式（mono abbr + serif name + sans desc）+ 中央 callout，或 flowchart skill 截图 | 系统架构、平台关系、hub-and-spoke |
...
所有通过 `fill-template.js` 生成的产物自动内置 Doc / Deck 双模式，无需额外操作。
...
**URL hash 直达**：`#deck:overview` 刷新后直接进入 Deck 模式定位到该页。Doc 模式下退出 Deck 会清除 hash。
...
sys.path.insert(0, os.path.join(_WS, '.claude/skills/ppt/scripts'))
...
**技术选型**：python-docx（参考模板 `scripts/gen-notes-docx.py`）
