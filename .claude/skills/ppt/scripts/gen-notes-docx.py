...
本文件直接运行 = 用文件内自带的示例 NOTES 出一份 demo docx。

参数：无命令行参数——所有配置改文件内常量：
  - NOTES 列表（「数据」节）＝ 每页口播内容，每次使用时整体替换
  - OUTPUT_PATH 常量 ＝ docx 输出路径（默认 ../deliverables/ppt-{主题}-notes-v1.docx，按需改）
