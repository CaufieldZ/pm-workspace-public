...
function fillTemplate(options) {
   ...
   * @param {string} [options.theme] - 主题名（默认 claude-native = tokens.css 默认值；其余 = 追加 _shared/claude-design/themes/{theme}.css 覆盖）
   * @param {Object} [options.notes] - 演讲者逐字稿 { tabId: '<p>逐字稿 HTML</p>' }，注入到 window.__PPT_NOTES__ 给 S 键 popup 使用
  ...
  // 注入 Claude Design tokens（CSS 变量源头唯一，html-pipeline.md §三）
  const tokensCSSPath = path.join(SKILL_DIR, '..', '_shared', 'claude-design', 'tokens.css');
  let tokensCSS = '';
  if (fs.existsSync(tokensCSSPath)) {
    tokensCSS = fs.readFileSync(tokensCSSPath, 'utf8');
  }

  // 追加 theme overrides（claude-native = tokens.css 默认值，不追加；其余主题追加覆盖 --cd-*）
  if (theme && theme !== 'claude-native') {
    const themePath = path.join(SKILL_DIR, '..', '_shared', 'claude-design', 'themes', `${theme}.css`);
    if (fs.existsSync(themePath)) {
      tokensCSS += `\n\n/* === Theme override: ${theme} === */\n` + fs.readFileSync(themePath, 'utf8');
    } else {
      console.warn(`⚠️  Theme '${theme}' not found at ${themePath}, falling back to claude-native default tokens`);
    }
  }

  template = template.replace('/* __TOKENS_CSS__ */', tokensCSS);

  // 注入 animations.css（27 个 CSS 动画，来自 lewislulu/html-ppt-skill）
  const animationsCSSPath = path.join(SKILL_DIR, 'assets', 'animations.css');
  if (fs.existsSync(animationsCSSPath)) {
    template = template.replace('/* __ANIMATIONS_CSS__ */', fs.readFileSync(animationsCSSPath, 'utf8'));
  } else {
    template = template.replace('/* __ANIMATIONS_CSS__ */', '');
  }
  ...
  // 注入 NOTES（演讲者逐字稿 · 补强 1）
  const notesJSON = notes ? JSON.stringify(notes) : '{}';
  template = template.replace('/* __NOTES_PLACEHOLDER__ */', `window.__PPT_NOTES__ = ${notesJSON};`);
