...
<div class="page active film-grain">
  ...
  <div class="grid4 gap-4">
    ...
    <div><div class="eyebrow">02 CONTEXT</div><p class="mt-2" style="color:var(--t2);font-size:13px;">baseline 沉淀</p></div>
...
<div class="page active">
  ...
  <div class="pullquote mt-8">两轨混用最佳：AI 方案讨论 → baseline → Claude Code 批量产出。<div class="pullquote-cite">— PM-WORKFLOW 最佳实践</div></div>
</div>
```
...
<div class="page active">
  ...
  <h2 class="display display-lg">项目真相源长出全部产出物</h2>
  <p class="lede">baseline 是项目当前态的 living 真相源，scene-list 锁场景编号，下游交互大图 / 原型 / PRD 全部从它生长。</p>
  <div class="hairline"></div>

  <!-- 1. 全景图开场（editorial 树形，不用彩色 gradient border）-->
  <div class="section-label">§ 2.1 · 从 baseline 长出的全部产出物</div>
  <div style="text-align:center;padding:48px 0;">
    <div class="display display-md">baseline</div>
    <div class="figure-lbl mt-2">ROOT · 唯一真相源</div>
    <div class="hairline-sm" style="margin:24px auto;"></div>
    <div class="grid4 gap-4">
      <div><div class="eyebrow">SCENE</div><div style="color:var(--t2);font-size:12px;">场景清单</div></div>
      ...
      <div><div class="eyebrow">PRD</div><div style="color:var(--t2);font-size:12px;">需求文档</div></div>
    </div>
  </div>
  ...
  <div class="grid1 gap-0">
    ...
    <div class="py-3"><span class="eyebrow eyebrow-accent">STEP 03</span><span style="color:var(--t1);margin-left:16px;">沉淀为 baseline + scene-list</span></div>
  </div>

  <!-- 4. 9 章折叠 — 内部细节 -->
  <div class="section-label">§ 2.4 · baseline 模块结构（细节）</div>
  <div class="accordion">
    <div class="acc-header" onclick="toggleAcc(this)"><span>点开查看 baseline 完整模块</span><span class="arrow">▶</span></div>
    <div class="acc-body"><!-- baseline 模块清单 --></div>
  </div>
</div>
```
...
<div class="page active">
  ...
  <div class="grid1 gap-0">
    ...
    <div class="py-3" style="border-bottom:1px solid var(--border2);"><span class="eyebrow">03</span><span style="color:var(--t1);margin-left:12px;">check_imap.sh / check_proto.sh 自检</span></div>
  ...
  <table class="cmp-table">
    ...
    <tbody>
      ...
      <tr><td style="color:var(--t1);font-weight:700;">自检</td><td>check_imap.sh / check_proto.sh</td><td>impact-check + cross-check</td></tr>
    </tbody>
  </table>
...
<div class="page active">
  ...
  <div class="grid4 gap-4">
    <div><div class="eyebrow">LAYER 01</div><h3 class="display display-md mt-2">规则层</h3><p style="color:var(--t2);font-size:12px;">CLAUDE.md / pm-methodology / artifact-conventions</p></div>
...
<div class="page active">
  ...
  <div class="py-6" style="border-bottom:1px solid var(--border2);">
    ...
    <p class="lede">推理深度 + 长上下文。适合 baseline 讨论、架构决策、复杂变更推演。</p>
  </div>
...
<div class="page active">
  ...
  <table class="cmp-table">
    ...
    <tbody>
      ...
      <tr><td style="color:var(--t1);font-weight:700;">prd</td><td>pipeline</td><td>.md</td><td>scene-list</td><td>baseline + delta</td></tr>
      <!-- ...剩余 16 行 -->
    </tbody>
  </table>
...
**节奏铁律**（SKILL.md Step 2 同步）：

1. 相邻两页禁止同一主组件（占 ≥ 50% 面积的算主组件）
2. 每 4-6 内容页插入一个 `page-hero` 或 `page-split` 呼吸页
3. 竖向 `pipe`（≥ 5 步）与横向 `flow-h`（≤ 4 步）交替
4. emoji 密度：`section-label` 不放 emoji，每页 ≤ 3 个，只在 `page-title` / `pipe-icon`
5. 文字密度：单页 renderer ≤ 60 行；金句 / 判断句用 `quote-block` 单独成段
6. 分步揭示：呼吸页金句和清单页条目可加 `data-step`，Doc 模式自动全显
