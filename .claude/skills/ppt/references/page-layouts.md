...
> ⚠️ **HTML 样例已归档**至 `assets/archive/page-layouts/`（31 个单页 HTML）。本文 layout 名 + 代码块仍可 grep 参考；纯 deck 范式优先用 `deck-template.html` 内置组件（见 `deck-grammar.md §一`）。
...
**推荐动画**：display-xl 加 `anim-blur-in`（封面 reveal），lede 加 `anim-fade-up`
...
**推荐动画**：整页 `anim-perspective-zoom` 或 `anim-cube-rotate-3d`（章节转场感）
...
**推荐动画**：figure-num 加 `counter` 类 + `data-to="N"`（数字累加），容器加 `anim-stagger-list`（多个数错开）
...
**推荐动画**：pullquote 加 `anim-fade-up`，figure 加 `anim-fade-right`（左右先后入场）
...
**推荐动画**：grid3 容器加 `anim-stagger-list`（各 figure 错开入场）
...
**推荐动画**：`pipe` 容器加 `anim-stagger-list`（各节点 + 箭头按顺序入场，强化流程感）
...
**推荐动画**：display-xl 大问号加 `anim-blur-in`（悬念感）
...
**推荐动画**：pullquote 加 `anim-fade-up` 或 `anim-spotlight`（金句聚光感）
...
**推荐动画**：左 before 加 `anim-fade-left`，右 after 加 `anim-fade-right`（对比张力）
...
**推荐动画**：figure 加 `anim-fade-right`，右侧 card 列表加 `anim-stagger-list`
...
## 附录 A · 31 lewislulu single-page layouts 索引

> 来源：[lewislulu/html-ppt-skill](https://github.com/lewislulu/html-ppt-skill) `templates/single-page/*.html`
> 已 copy 至 [`.claude/skills/ppt/assets/archive/page-layouts/`](../assets/archive/page-layouts/)
> 31 个独立 HTML 文件，每个带真实示例数据，浏览器直接打开可看 demo（但样式依赖 lewislulu 的 `--accent / --text-1` 等变量，本仓库不引入，故视觉会缺色——主要作为「`<section>` 结构 + class 命名 + 真实示例数据」的代码参考）
> **使用方式**：模型 / PM 选某个 layout 时 `Read .claude/skills/ppt/assets/archive/page-layouts/{name}.html`，抽 `<section class="slide">` 块结构和示例数据，按 ClaudeDesign 体系（`page-hero` / `figure` / `grid3` / `cmp-table` 等组件）改写后塞进 PAGE_RENDERERS

### A.1 开场 / 转场（3 个）

| 文件 | 用途 |
|---|---|
| `cover.html` | 封面：kicker + 大标题 + lede + pill 行 |
| `toc.html` | 目录：2×3 编号卡片 |
| `section-divider.html` | 章节切分：大编号 + 主题（02 · Theme）|

### A.2 文字密集（4 个）

| 文件 | 用途 |
|---|---|
| `bullets.html` | 经典 bullet 列表，每条卡片包裹 |
| `two-column.html` | 概念 + 例子左右对照 |
| `three-column.html` | 三等列 + 图标（三原则 / 三支柱）|
| `big-quote.html` | 全屏金句，editorial-serif 风 |

### A.3 数据 / 图表（7 个）

| 文件 | 用途 |
|---|---|
| `stat-highlight.html` | 单个巨大数字 + 副标，配 `counter-up` 动画 |
| `kpi-grid.html` | 一行 4 个 KPI + 涨跌 delta |
| `table.html` | 数据表，hover 行高亮，右对齐数字 |
| `chart-bar.html` | Chart.js 柱状图（主题色感知）|
| `chart-line.html` | Chart.js 双线图 + 区域填色 |
| `chart-pie.html` | Chart.js 环形图 + takeaways 卡片 |
| `chart-radar.html` | Chart.js 雷达图（两产品 6 维对比）|

### A.4 代码 / 终端（3 个）

| 文件 | 用途 |
|---|---|
| `code.html` | 语法高亮代码块（highlight.js）|
| `diff.html` | 手写 +/- diff 视图 |
| `terminal.html` | 终端窗口仿真（traffic-light 头）|

### A.5 图表 / 流程（4 个）

| 文件 | 用途 |
|---|---|
| `flow-diagram.html` | 5 节点 pipeline + 箭头 + 1 个高亮节点 |
| `arch-diagram.html` | 3 层架构网格 |
| `process-steps.html` | 4 个编号步骤卡片 |
| `mindmap.html` | 放射型脑图 + SVG path-draw 动画 |

### A.6 规划 / 比较（6 个）

| 文件 | 用途 |
|---|---|
| `timeline.html` | 5 节点水平时间线 |
| `roadmap.html` | NOW / NEXT / LATER / VISION 四列 |
| `gantt.html` | 12 周 5 轨道甘特图 |
| `comparison.html` | Before vs After 两面板 |
| `pros-cons.html` | 优劣对比双卡 |
| `todo-checklist.html` | 勾选清单（已选 / 未选状态）|

### A.7 视觉（2 个）

| 文件 | 用途 |
|---|---|
| `image-hero.html` | 全屏 hero + Ken Burns 渐变背景 |
| `image-grid.html` | 7 单元 bento 网格（渐变占位）|

### A.8 收尾（2 个）

| 文件 | 用途 |
|---|---|
| `cta.html` | Call-to-action：大渐变标题 + 按钮 |
| `thanks.html` | 「Thanks」结尾 + confetti 爆裂 |

### Picking a layout 决策树

- **开场**：`cover` → 通常接 `toc`
- **章节切分**：每个主章节前用 `section-divider`
- **核心内容**：`bullets` / `two-column` / `three-column`
- **展示数字**：单个用 `stat-highlight`，4 个用 `kpi-grid`
- **展示走势 / 占比**：`chart-bar` / `chart-line` / `chart-pie` / `chart-radar`
- **展示变化 / 差异**：`comparison` / `diff` / `pros-cons`
- **展示计划**：`timeline` / `roadmap` / `gantt` / `process-steps`
- **展示架构**：`arch-diagram` / `flow-diagram` / `mindmap`
- **代码 / 演示**：`code` / `terminal`
- **收尾**：`cta` → `thanks`
...
→ 见 `references/gold-snippets.md §7 节奏编排`（10 页示例 + 推荐序列 + 节奏铁律六条 + 反面教材，单一来源）。
