...
usage() {
  cat <<'EOF'
promo-kit Step B 自检：上线宣发三件套（launch-*-video/-grid/-copy.md）的结构 / 讲人话 / 合规红线机械检查。
无第三方依赖，纯 bash + grep；按文件名后缀（-video / -grid / -copy）自动选校验集。
...
形态结构（视频三段式 + 文案列 / 短文案三段 + 品牌 Tag / 4 宫格行数）；WARN（提示）：卖点句 > 20 字。
