...
> "当你想'加一下会更好看'时——那通常是 AI slop 的征兆。先做最简版本，忍住，等真的需要再加。"
...
### 印刷文档（PRD docx / 数据报告 / 任何线下打印物）

| 元素 | 规格 |
|------|------|
| 正文 | **10-11pt**（最小 10pt，最大 12pt）|
| 标题 1 | **24pt** |
| 标题 2 | **18pt** |
| 标题 3 | **14pt** |
| Caption / 注释 | **8-9pt** |
...
> PRD v2 起主形态是 md（见 prd skill），印刷规范只在 6 个老 docx 维护、数据报告导出、PPT 印刷版用。
...
## CSS 变量源头唯一（HTML 产出物强制）

[tokens.css](./tokens.css) 是 `--cd-*` 主 token（`--cd-bg` / `--cd-ink` / `--cd-ink-{82,58,40,18}` / `--cd-hairline` / `--cd-accent*` / `--cd-serif-cn` / `--cd-sans` / `--cd-mono` 等）的**唯一定义源**。注入方式两种任选：

- **占位替换**（PPT skill 模式）：HTML/CSS 模板里写 `/* __TOKENS_CSS__ */` 占位，fill / gen 脚本 `fs.readFileSync('.claude/skills/_shared/claude-design/tokens.css')` 替换
- **拼接注入**（SOP / 大文档 gen 模式）：gen 脚本 `parts.push('<style>')` 之后先 push tokens.css 内容再 push 项目 styles.css

**禁止**：

- 在任何 skill assets / sop-src / gen 脚本里手抄 `--cd-bg: #1F1F1E` / `rgba(228,226,214,*)` 字面值
- 在 `:root` 块里重复定义 cd-* 主 token

项目级扩展 token（`--cd-surface2` / `--cd-ok` / chart palette / 非标 alpha 档如 0.04 / 0.28）放在独立 `:root {}` 块里、用 `var(--cd-ink)` 引用，禁手抄数值。
...
> PRD / 数据报告作者的详细操作手册（placeholder 写法 `[数据待填：xxx]` / `[用户引言待填]` + 业务规则完备性自检 + 数值来源标注）见 [content-slop-ban.md](./content-slop-ban.md)。
