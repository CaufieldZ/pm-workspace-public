...
- `.claude/skills/flowchart/references/gen_flow_base.py`(base 生成器 API)
...
sys.path.insert(0, str(Path(__file__).parents[3] / ".claude/skills/flowchart/references"))
...
### 截图插入 PRD
标准流程:Playwright 截图 → Pillow 圆角透明(可选) → docx 插入。参考 prd skill 的截图脚本模板(`.claude/skills/prd/references/` 下的 Step 4 说明),增加 selector 参数截取 `.whiteboard` 而非 `.phone`。
