...
if run_cat 13; then
  ...
  for updater in \
    ".claude/skills/interaction-map/scripts/update_imap_base.py:ImapUpdater" \
    ".claude/skills/prototype/scripts/update_proto_base.py:ProtoUpdater" \
    ".claude/skills/ppt/scripts/update_ppt_base.py:PptUpdater"; do
    upath="${updater%%:*}"
    uclass="${updater##*:}"
    [ -f "$upath" ] || continue
    udir=$(dirname "$upath")
...
" 2>/dev/null; then
  ...
  for gen in \
    ".claude/skills/interaction-map/scripts/gen_imap_skeleton.py:generate_skeleton" \
    ".claude/skills/prototype/scripts/gen_proto_skeleton.py:generate_skeleton" \
