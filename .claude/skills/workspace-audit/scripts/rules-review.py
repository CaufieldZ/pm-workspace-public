...
用法：
    python3 .claude/skills/workspace-audit/scripts/rules-review.py --model <模型版本>
    python3 .claude/skills/workspace-audit/scripts/rules-review.py --model <模型版本> --dry-run

参数：
    --model <版本>   当前模型版本（记入报告头），必填
    --dry-run        只打印统计 + 前 5 条规则预览，不写报告文件

产物: deliverables/rules-review-YYYY-MM-DD.md（--dry-run 不写文件）。

前置: 无强依赖；.claude/logs/usage.jsonl 缺失时 hook 统计为空（报告仍出）。--help 不需要。

退出码: 0 = 跑完（写文件或 dry-run）；2 = 参数用法错（如缺 --model）。
"""
...
def main():
    ...
    parser.add_argument("--model", required=True, help="当前模型版本（记入报告头）")
    parser.add_argument("--dry-run", action="store_true",
                        help="只打印统计 + 前 5 条规则预览，不写报告文件")
