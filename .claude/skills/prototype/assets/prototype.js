...
function switchGlobalView(idx) {
  ...
  var secs = document.querySelectorAll('.gnav-view-section');
  secs.forEach((v, i) => v.classList.toggle('gnav-active', i === idx));
  // 兜底：激活的 view 若没有可见页（被别的 view goPage 清过），显示其首页，避免黑屏
  var sec = secs[idx];
  if (sec && !sec.querySelector('.p-page.show')) {
    var first = sec.querySelector('.p-page');
...
// ── 页面跳转（scope 到当前 active view，避免跨 view 清空导致黑屏）──
function goPage(name) {
  var sec = document.querySelector('.gnav-view-section.gnav-active') || document;
  sec.querySelectorAll('.p-page').forEach(p => p.classList.remove('show'));
  var target = sec.querySelector('.p-page[data-page="' + name + '"]')
            || sec.querySelector('#page-' + name)
...
// ── 抽屉（scope 到当前 active view 的 shell，多 view 不撞 id）──
function openDrawer() {
  var sec = document.querySelector('.gnav-view-section.gnav-active') || document;
  var o = sec.querySelector('.p-overlay'), d = sec.querySelector('.p-drawer');
