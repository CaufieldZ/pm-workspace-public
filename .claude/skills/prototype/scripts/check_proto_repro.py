...
何时用：audit cat24 / proto-drift-warn hook 链调用；hook 指纹比对报可疑时
人工复跑拿逐版本精确结论（全量重建，较慢）。
...
封版豁免：`.proto-lock.json` 带 `frozen=true` 的版本跳过比对（决策记录：
.claude/decisions/implemented/2026-08-25-proto-drift-frozen.md）。

参数：
    [产线名...]   可选；只重建比对这些产线（projects/ 下目录名，可多个，默认全部）
    --strict      漂移时 exit 2（hook 用）

前置：产线侧已建共享场景库（projects/{产线}/scripts/src/registry.py）——
无则输出 skip 并 exit 0；重建落 tempfile 临时目录，不碰已交付产物。
