"""Step C 标杆对照自检 · prototype skill：候选 HTML 对照标杆过视觉门。

何时用：原型 build 完成后交付前跑（prototype-audit hook 自动跑摘要，
人工复跑看逐项明细）；SKILL.md 标杆对照强制红线的执行载体。
...
参数：
  html        候选 HTML 文件路径（待检的原型产物）
  --baseline  标杆 HTML 路径（可选；存在时在报告头部展示标杆名，供人工对照）
  --paradigm  范式 key：single-phone-scenes / single-phone-no-nav /
              multi-view-gnav / single-end（默认按 HTML 内容自动探测）

前置：候选 HTML 本地文件；bs4 可选——缺失时 V2 壳内素材检查跳过，其余规则照跑。

退出码：
  0 — 全通过（含 warn 项）
  非 0 — 有 fail，数字 = fail 项数（上限 99）
"""
import argparse
import re
import sys
...
EPILOG = """\
示例：
  # 交付前自检（hook 自动跑摘要，人工复跑看逐项明细）
  python3 .claude/skills/prototype/scripts/audit_against_baseline.py projects/<产品线>/deliverables/<季度>/<版本>/proto-<产品线>-<版本>-<端>.html
  # 显式指定范式
  python3 .claude/skills/prototype/scripts/audit_against_baseline.py deliverables/proto-xxx.html --paradigm single-phone-scenes
  # 对照标杆（check_paradigm.py 输出的标杆路径）
  python3 .claude/skills/prototype/scripts/audit_against_baseline.py deliverables/proto-xxx.html --baseline projects/<产品线>/archive/<季度>/<迭代>/deliverables/<原型文件>.html
"""
...
def main():
    ...
    ap.add_argument("html", help="候选 HTML 文件路径（待检的原型产物）")
    ap.add_argument("--baseline",
                    help="标杆 HTML 路径（可选；存在时在报告头部展示标杆名，供人工对照）")
    ap.add_argument("--paradigm",
                    help="范式 key：single-phone-scenes / single-phone-no-nav / "
                         "multi-view-gnav / single-end（默认按 HTML 内容自动探测）")
    args = ap.parse_args()
