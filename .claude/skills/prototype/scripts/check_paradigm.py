"""Step 0 范式门 · prototype skill：开原型前定范式、标杆与必读素材。

何时用：新项目进 prototype 流程第一步（出骨架前）手动跑；bash-gate 拦
「未跑范式门就 build」时也提示跑本脚本。

读真相源（baseline）+ scene-list.md，推断端构成，输出推荐范式 + 标杆 + 必读 references。

参数：
    project   项目名（projects/ 下目录名，见 projects/ 下目录名）

前置：projects/{项目名}/ 已有 baseline 真相源（prd-{产品线}-baseline.md，
经 lib.truth_source 解析）+ scene-list.md；--help 本身不需要它们。
...
EPILOG = """\
示例：
    # Step 0 定范式（需先有 baseline + scene-list.md）
    python3 .claude/skills/prototype/scripts/check_paradigm.py <产品线>
    python3 .claude/skills/prototype/scripts/check_paradigm.py <产品线>
"""
...
PARADIGMS = {
    "single-phone-scenes": {
        ...
        "baseline": "projects/<产品线>/archive/<季度>/<迭代>/deliverables/<原型文件>.html",
        "alt_baselines": [
...
def main():
    ...
    ap.add_argument("project",
                    help="项目名（projects/ 下目录名，见 projects/ 下目录名）")
    args = ap.parse_args()
