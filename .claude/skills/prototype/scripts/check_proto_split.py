#!/usr/bin/env python3
"""prototype src/scenes 分场景拆分门：校验 proto-*.html 背后有分场景源码。

何时用：prototype-split-gate hook 自动调（proto HTML 落盘触发）；
人工复跑用于排查被 hook 拦下的拆分缺失。
...
参数：
    files      proto-*.html 路径，可多个（非 .html 后缀自动跳过）
    --strict   阻断模式：未拆分 exit 2（hook 用）；默认 exit 1 仅告警

前置：待检 HTML 已产出。从 HTML 路径向上最多 6 层找 scripts/src/scenes/ 或
src/scenes/（覆盖 deliverables/{季度}/{版本}/ 与项目根两种落点）。

退出码：
    0 — 找到 src/scenes/*.py（已拆分）
    1 — 未拆分但未传 --strict
    2 — 传 --strict 且未拆分
    临时绕过（仅 hook 侧生效，需说明理由）：SKIP_PROTOTYPE_SPLIT_GATE=1
"""
import argparse
import sys
...
EPILOG = """\
示例：
    # 人工复跑单文件（hook 拦下后看已查候选目录）
    python3 .claude/skills/prototype/scripts/check_proto_split.py projects/<产品线>/deliverables/<季度>/<版本>/proto-<产品线>-<版本>-<端>.html
    # 多文件 + 阻断模式（hook 调用形态）
    python3 .claude/skills/prototype/scripts/check_proto_split.py deliverables/<季度>/<版本>/proto-*.html --strict
"""
...
def main():
    ...
    ap.add_argument("files", nargs="+",
                    help="proto-*.html 路径，可多个（非 .html 后缀自动跳过）")
    ap.add_argument("--strict", action="store_true",
                    help="阻断模式：未拆分 exit 2（hook 用）；默认 exit 1 仅告警")
    args = ap.parse_args()
