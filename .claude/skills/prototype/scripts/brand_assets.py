...
直跑自检（demo，正式用法走 import）：
    python3 .claude/skills/prototype/scripts/brand_assets.py
    无参数；输出 3 个示例 SVG 并逐个做 XML 语法校验。

前置：无。

退出码：
    0 — 自检通过（示例 SVG 语法校验全过）
    非 0 — 自检异常（SVG 语法错抛栈退出）
"""
...
if __name__ == '__main__':
    # 自检：输出示例 HTML 供肉眼验证；-h/--help 只打印用法，不跑校验。
