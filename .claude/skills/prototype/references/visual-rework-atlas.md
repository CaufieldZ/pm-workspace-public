...
- **手机壳半屏浮层的截图**：浮层内容常高过 464px 可视区，下半区要在 `extra_shots` 的 `setup` 里设 `scrollTop` 单独截一张——偏移量预留分区标题高度，否则截到的是标题不是内容。
...
- **复用既有组件拼新卡类型时逐元素核语义标记**：B/S 买卖点是仓位卡专属语义，收益率类卡片连带复用会显示成买卖信号——组件里每个 class / 图标 / 着色都先当它「有含义」，再决定留不留。
