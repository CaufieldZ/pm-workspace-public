...
> 本文件薄。重头戏：① interaction-map/references/biz-*.md（Figma 真品 992 行业务组件） ② 本文 § B 真品组件代码（示例 Figma 直抽，直接 copy-paste 级）。
>
...
| 通用 Phone 内部组件（表单 / 空态 / 列表 / Tab） | `components-core.md` §1（单一信息源，Prototype 不再维护重复副本）|
...
### B4. 示例 App 底部导航 · 双层结构（375×134）

来源：[Figma App-Sparkle node 12547-22326](https://www.figma.com/design/ewHDBGMTwKVgzcNW3B4FM0/?node-id=12547-22326) · 本地：[figma-anchors/app-bottom-nav.png](../assets/figma-anchors/app-bottom-nav.png) · spec：[figma-anchors/app-bottom-nav.spec.txt](../assets/figma-anchors/app-bottom-nav.spec.txt)

**关键反 v2 错觉**：示例底部导航不是单层 5-tab，是**双层 + 圆形交易主按钮**：
...
```css
.p-tbar{position:absolute;left:0;right:0;bottom:0;background:#191919;border-radius:20px 20px 0 0;font-family:'HarmonyOS Sans SC',-apple-system,sans-serif;}
.p-tbar-top{display:flex;align-items:center;height:56px;padding:0 16px;gap:18px;}
.p-tbar-top .t{font-size:16px;font-weight:500;line-height:20px;color:#828282;position:relative;padding:8px 0;}
.p-tbar-top .t.on{color:#FFFFFF;}
.p-tbar-top .t.on::after{content:'';position:absolute;left:50%;transform:translateX(-50%);bottom:6px;width:16px;height:3px;background:#007FFF;border-radius:1.5px;}
.p-tbar-top .caret-up{margin-left:auto;color:#FFFFFF;font-size:14px;cursor:pointer;}
.p-tbar-main{display:flex;height:44px;border-top:1px solid rgba(0,0,0,.1);}
.p-tbar-main .tab{flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;color:#828282;}
.p-tbar-main .tab.on{color:#FFFFFF;}
.p-tbar-main .tab span{font-size:10px;font-weight:400;line-height:14px;}
.p-tbar-main .tab-fab{position:relative;}
.p-tbar-main .tab-fab i{width:44px;height:44px;border-radius:50%;background:#FFFFFF;display:flex;align-items:center;justify-content:center;position:absolute;top:-12px;}
.p-tbar-main .tab-fab span{margin-top:32px;}
...
1. **Figma node X-Y · 本地 ../assets/figma-anchors/xxx.png · spec：xxx.spec.txt**（最高权威 — 示例自家真品）
...
**所有 Platform C prototype 必须**：

```css
font-family:'HarmonyOS Sans SC','Noto Sans SC',-apple-system,sans-serif;
```

HarmonyOS Sans SC 是 示例官方 Figma 全栈用的中文字体（确认见所有节点 `style.fontFamily`）。Noto Sans SC 是 fallback。把 HarmonyOS Sans SC 漏掉 = 视觉差一档。

数字字体走 'IBM Plex Mono' / 'JetBrains Mono'，与 HarmonyOS Sans SC 配合。

**动态数字必须 `font-variant-numeric:tabular-nums`**（来源：livestream 2.4 原型）：价格 / 倒计时 / 在线人数 / 金额等会刷新的数字一律加。不加的后果：数字宽度随内容变化，每次刷新整行布局抖动。字号小的静态数字（副标题里的一次性数字）可省。

## E. Figma 真品扩展规则

后续遇到本文件未覆盖的 示例 UI 组件（卡片 / 抽屉 / chip / 下拉），同样规则：
...
不准跳过 1-3 步直接写 — 模型对 示例视觉的「印象」永远比真品差一档。
