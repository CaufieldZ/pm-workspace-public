...
无 IMAP，但 scene-list.md + baseline 场景章描述齐全
...
```bash
python3 .claude/skills/prototype/scripts/pre_proto_phone_shots.py \
...
- ✅ baseline 场景章对该场景有「布局描述 + 必备组件清单 + 状态变体说明」三段式描述（≥ 200 字 / 场景）

至少 1 项满足；3 项都没 → 停下来问用户补，禁止继续。

### D2. Step A 前置动作

1. 模型逐场景列出依据来源（截图路径 / Figma node / baseline 段落锚点），写到 `projects/{项目}/inputs/scene-anchors.md`
...
- **C-3 战绩 TAB**：anchor=baseline L420-L530（200 字布局 + 组件 + 状态描述）
...
- ≤ 3 场景的小项目允许全程依赖 baseline 场景章 + 竞品截图
