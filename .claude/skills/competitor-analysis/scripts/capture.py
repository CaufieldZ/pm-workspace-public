...
用法:
  python3 capture.py --output-dir <dir>                  # 默认录 60s
  python3 capture.py --output-dir <dir> --duration 30    # 录 30s
  python3 capture.py --output-dir <dir> --fps 4          # 提高抽帧频率

输出: JSON 到 stdout，格式 {"captures": [{path, timestamp}], "total": N}；
有效截图按 001.png / 002.png … 落 --output-dir，过程日志走 stderr。

参数:
  --output-dir <dir>   截图保存目录（必填，不存在自动建）
  --duration <秒>      录制时长，默认 60
  --fps <N>            抽帧频率，默认 2

前置: macOS + 本地设备——iPhone 镜像窗口已打开（采集目标）；ffmpeg、sips、swift
（macOS 内置）。录制期间勿移动镜像窗口。--help 不需要以上前置。

退出码: 0 = 采集完成；1 = 未找到 iPhone 镜像窗口 / 录制失败；2 = 参数用法错。
"""
import argparse
import json
import os
...
def pixel_diff_pct(pixels_a, pixels_b):
    ...
    if len(pixels_a) != len(pixels_b):
        return 100.0
    diff = sum(1 for a, b in zip(pixels_a, pixels_b, strict=True) if a != b)
...
def main():
    ...
    print("   录制期间请勿移动 iPhone 镜像窗口。", file=sys.stderr)
    ...
    print("📼 录制完成，正在提取有效帧...", file=sys.stderr)
