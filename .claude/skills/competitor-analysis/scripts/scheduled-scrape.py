...
用法:
  python3 scheduled-scrape.py --all                       # 全部交易所 + 媒体（crontab 走此档）
  python3 scheduled-scrape.py --platforms <平台1>,<平台2>  # 指定交易所公告
  python3 scheduled-scrape.py --media                     # 仅媒体
  python3 scheduled-scrape.py --platforms <平台> --media  # 交易所 + 媒体

参数:
  --platforms <p1,p2>  逗号分隔的平台名，取值见 assets/url-registry.json 顶层键（无 _ 前缀者）
  --media              采媒体源（url-registry.json 的 _media 节）
  --all                全部交易所 + 媒体
  三者至少传一个，否则按用法错退出。

产物: references/competitors/<平台>/announcements/YYYY-MM-DD.md 与
references/competitors/_media/<媒体>/YYYY-MM-DD.md；同日同平台多源追加写，内容不变不重写。
结果 JSON 输出到 stdout（{"scraped","new","errors","details"}）。

前置: playwright + chromium 已装；assets/url-registry.json 在。--help 不需要（playwright 延迟 import）。

退出码: 0 = 跑完（单页失败不中断，看 JSON errors 计数）；2 = 参数用法错。
"""
...
WORKSPACE_ROOT = SCRIPT_DIR.parents[3]  # .claude/skills/competitor-analysis/scripts -> root
REGISTRY_PATH = ASSETS_DIR / "url-registry.json"
AUTH_DIR = ASSETS_DIR / "auth"
...
async def extract_jsonld(page):
    ...
    try:
        ...
        print(f"[WARN] extract_jsonld failed: {_e}", file=sys.stderr)
        return None
...
async def main():
    ...
    parser.add_argument("--platforms", type=str, help="逗号分隔平台名，取值见 assets/url-registry.json 顶层键")
    parser.add_argument("--media", action="store_true", help="采媒体源（url-registry.json 的 _media 节）")
    parser.add_argument("--all", action="store_true", help="全部交易所 + 媒体")
