...
> 触发：用户说「采集 / 截 / 抓 / 调研」时按需读。「分析 / 调研」走 SKILL.md Step 1 不读本 playbook。

---

## 采集模式（原 intel-collector 能力）

自动采集竞品交易所的 APP/Web 截图、公告内容、以及 Crypto 媒体报道，归档到 `references/competitors/`。是「拆解分析」的上游素材供给——先采集，再分析。
...
```bash
python3 .claude/skills/competitor-analysis/scripts/capture.py \
...
**目标**：核心是避开 `state` 和 `get html` 这两个 token 黑洞。
...
| `state` | 2k–8k | 每 session 最多 2 次，仅用来认入口 |
| `get html --selector` | 1k–15k | 禁止直接给 LLM 理解，只允许落盘后 grep |
...
#### Step C：Vision 过滤 + 自动命名（APP/Web 模式）
...
**脚本位置**：`.claude/skills/competitor-analysis/scripts/scheduled-scrape.py`

```bash
# 手动运行
python3 .claude/skills/competitor-analysis/scripts/scheduled-scrape.py --all          # 全部
python3 .claude/skills/competitor-analysis/scripts/scheduled-scrape.py --platforms okx # 指定平台
python3 .claude/skills/competitor-analysis/scripts/scheduled-scrape.py --media        # 仅媒体
```
