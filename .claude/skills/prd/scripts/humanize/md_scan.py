...
# ── 验收复述规格（acceptance_echo）──────────────────────────────
# 「验收不复述规格」是写作纪律，行形状检查抓不到：复述句单看每行都合格。
# 作用域限在同一场景单元内（跨单元同词不同事是正常的），把验收 bullet 与本单元
# 规格 bullet 两两比字符 bigram 相似度，超阈值即 WARN。
# 阈值 0.5 按真实语料标定（7 份 delta / baseline 共 ~210 条验收，命中 24 条，人工核真阳性 ~8 成；
# 残余假阳性集中在「同结构不同分支」，如取消申请 vs 关闭直播）。
_ACK_ECHO_THRESHOLD = 0.5
_ACK_MIN_LEN = 10  # 短断言（「按钮可点」）bigram 太少，比值噪声大
_ACK_TAG_RE = re.compile(r"<[^>]+>")
_ACK_NOISE_RE = re.compile(r"[「」『』（）()\[\]\s，。、；：·/／\-—…·*]+")
_ACK_LABELS = ("显示逻辑", "显示要素", "交互", "验收")
# 三种验收形态：块内 <strong>验收</strong> / 三列表 <th>验收</th> 列 / md 原生 **验收标准**：
_ACK_STRONG_RE = re.compile(r"<strong>\s*验收[^<]*</strong>")
_ACK_TH_RE = re.compile(r"<th>\s*验收[^<]*</th>")
_ACK_MD_RE = re.compile(r"^\s*\*\*验收[^*]*\*\*\s*[：:]")
_SPEC_STRONG_RE = re.compile(r"<strong>")
_SPEC_MD_RE = re.compile(r"^\s*\*\*[^*]+\*\*\s*[：:]")
_LI_RE = re.compile(r"<li>")
_MD_BULLET_RE = re.compile(r"^\s*[-*]\s+(\[[ xX]\]\s*)?\S")
_UNIT_SPLIT_RE = re.compile(r"^(#{2,4}\s|<table>)")


def _ack_norm(s: str) -> str:
    """剥标签 / 三段式标签词 / 标点空白，只留语义字符。"""
    s = _ACK_TAG_RE.sub("", s)
    for lb in _ACK_LABELS:
        s = s.replace(lb + "：", "").replace(lb, "")
    return _ACK_NOISE_RE.sub("", s)


def _ack_sim(a: str, b: str) -> float:
    """字符 bigram 相似度：取 Jaccard 与「a 被 b 覆盖率」的大者。

    覆盖率分支专抓「验收整句抄自规格长句里的一个分句」——这类 Jaccard 被长规格稀释，
    但覆盖率接近 1，是最典型的复述形态。
    """
    ...
    if not A or not B:
        return 0.0
    inter = len(A & B)
    return max(inter / len(A | B), inter / len(A))


def _scan_acceptance_echo(md_text: str) -> list[str]:
    """场景单元内验收 bullet 与规格 bullet 近重复（WARN）。

    单元边界：`<table>`（左图右文场景块）或 h2/h3/h4 标题（md 原生场景节）。
    单元内每行归类为 spec / ack / 忽略，再两两比。
    """
    hits: list[str] = []
    lines = md_text.splitlines()
    # 先切单元：记录每个单元的 (起始行号, 行列表)
    ...
    for i, raw in enumerate(lines, 1):
        ...
        if not in_fence and _UNIT_SPLIT_RE.match(raw.strip()) and cur:
            units.append((cur_start, cur))
            cur_start, cur = i, [raw]
            continue
        if not cur:
            cur_start = i
    ...
    for base, unit in units:
        spec: list[tuple[int, str, str]] = []
        ack: list[tuple[int, str, str]] = []
        in_ack = False
        # 三列表：验收是独立列 → 定位它在 thead 里的序号，tbody 内按 <td> 计数归类
        ack_col = -1
        head = "\n".join(unit[:12])
        if _ACK_TH_RE.search(head):
            # 「验收」前的 <th> 数已含它自己的开标签 → 减 1 得 0-based 列序，对齐 td_idx
            ack_col = len(re.findall(r"<th>", head.split("验收")[0])) - 1
        td_idx = -1
        for off, raw in enumerate(unit):
            line_no = base + off
            s = raw.strip()
            if "<td>" in s:
                td_idx += 1
            if "</tr>" in s:
                td_idx = -1
            # 归类：进 / 出验收段
            if _ACK_STRONG_RE.search(s) or _ACK_MD_RE.match(raw):
                in_ack = True
                continue
            if in_ack and ack_col < 0:
                # 遇到下一个粗体组头（<strong>N. 区块名</strong> 或 md **段名**：）即离开验收段
                if _SPEC_STRONG_RE.search(s) or _SPEC_MD_RE.match(raw):
                    in_ack = False
            is_li = bool(_LI_RE.search(s))
            is_md_bullet = bool(_MD_BULLET_RE.match(raw)) and "|" not in s
            if not (is_li or is_md_bullet):
                continue
            text = _ack_norm(s)
            if len(text) < _ACK_MIN_LEN:
                continue
            as_ack = in_ack or (ack_col >= 0 and td_idx == ack_col)
            (ack if as_ack else spec).append((line_no, text, s))

        for aln, at, araw in ack:
            best = (0.0, 0, "")
            for sln, st, sraw in spec:
                r = _ack_sim(at, st)
                if r > best[0]:
                    best = (r, sln, sraw)
            if best[0] >= _ACK_ECHO_THRESHOLD:
                hits.append(
                    f"L{aln}: 验收复述规格（相似度 {best[0]:.2f}，对应规格 L{best[1]}）"
                    f" 验收「{_ACK_TAG_RE.sub('', araw)[:34]}」"
                    f" ← 规格「{_ACK_TAG_RE.sub('', best[2])[:34]}」"
                )
    return hits


def _scan_server_platform_tracking(md_text: str) -> list[str]:
    """埋点表「应埋点平台」列写入服务端 / 后台类（CMS）→ 命中（Platform C 只做 APP / Web 端埋点，见 prd-chapter-rules §三点八）。"""
    hits: list[str] = []
    for cell_text, _h2, header, line_no, row_dict in _gather_table_cells(md_text):
        if header != "应埋点平台":
            continue
        low = cell_text.lower()
        bad_kw = ("服务端" in cell_text) or ("后台" in cell_text) or ("cms" in low)
        if not bad_kw:
            continue
        ev = row_dict.get("事件英文名", "").strip("`")
        ctx = f"事件 {ev}" if ev else cell_text
        hits.append(f"L{line_no}: 应埋点平台={cell_text}（{ctx}）→ 只做 APP / Web 端埋点")
    return hits
...
def scan_prd_structural_md(md_text: str, scene_count: int = 0) -> dict:
    ...
    return {
        ...
        "acceptance_echo_hits": _scan_acceptance_echo(md_text),
        "server_platform_tracking": _scan_server_platform_tracking(md_text),
