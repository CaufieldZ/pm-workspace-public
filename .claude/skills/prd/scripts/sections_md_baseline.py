...
def build_delta_skeleton(info: dict) -> str:
    ...
    # 不进头部，落 §9（推送时整章剥离）。
    # 「状态」「内部项目系统」两格由 sync_hx_status.py 按内部项目系统回写，故固定摆第 3 列 key / 第 4 列 value
    # （值落行尾单元格，回写正则好锚），勿调列位。
    w.table(
        headers=["项", "内容", "项", "内容"],
        rows=[
            ["PRD 版本", version,
             "状态", PL("待排期 / 开发中 / 已上线 / 已合并（sync_hx_status.py 按内部项目系统回写，勿手填）")],
            ["拟制人 / 日期", PL("姓名 / YYYY-MM-DD"),
             "内部项目系统", PL("H 号（上线日期 / 状态唯一权威源）")],
            ["重要性 / 紧迫性", PL("高 / 中 / 低"),
             "迭代档位", tier_label],
    ...
    w.h1("7. 埋点与看板")
    w.h2("7.1 本轮新增事件")
    w.paragraph("事件名与属性名为本轮拟名，待行为分析平台注册后回填真名。多属性事件每属性一行，事件名 / 触发机制列重复，数据团队按事件英文名 group。")
    w.table(
