...
两种模式（`--mode`）：
    ...
    dedup        — 全文单个探针，问「哪条规则被完整讲了不止一次」+「哪句话把简单的事写复杂了」。
                   赘述是全文属性，不分 target；机械检测只能抓「验收复述规格」这一种形状，
                   跨章重复与繁复表述靠语义判断。
...
DEDUP_CLASSES: list[tuple[str, str]] = [
    ...
    ("简单的事写复杂",
     "有没有句子把一个简单的事写复杂了？两类：① 冗余——可删的括号补充、同义反复的破折号从句、"
     "叠在一起的限定条件；② 名词化压缩——把动作压成名词串、省略主语，字数不多但一遍读不懂"
     "（「导晒单」「分流向发布器预填」「系统主动触点」）。判据是「读者一遍读懂」："
     "把动作还原成「谁做什么」，去掉不影响开发测试的修饰。原句一遍能读懂的不要报。"),
]
...
- 只 Read 这一个文件：`{ctx_path}`（{prd_name} 的自包含全文 + scene-list）。
  {_ctx_hint(ctx_path)}——一次性 Read 整篇会被大文件门拦下，用 offset / limit 分页读到文末。
...
def _ctx_hint(ctx_path: Path) -> str:
    """context 文件规模提示：近千行的文件一次性 Read 会被大文件门拦，冷读者要分页读。"""
    try:
        n = sum(1 for _ in ctx_path.open(encoding="utf-8"))
    except OSError:
        return "全文约千行"
    return f"全文约 {n} 行"
...
第 7 类与前 6 类不同：它找的不是「同一件事讲了几遍」，而是「一件事被讲复杂了」。
前 6 类按 A 段格式报，第 7 类按 B 段格式报。
...
## 不算写复杂（别报这些）
- 会影响开发或测试的限定条件（阈值 / 端侧 / 角色 / 时序 / 状态）——这些是承重信息，不是冗余。
- 业务规则配套的「理由：xxx」取舍说明（删了影响研发实施 / QA 写 case）。
- 原文已经足够简单的句子。
...
### B. 简单的事写复杂（第 7 类）—— 每条一个四件套
- **位置**：章节 + 原文片段（一句）
- **它其实在说什么**：一句话白话
- **改写**：用更简单的话说清同一件事（更短，或同样长但一遍读懂）
- **能砍什么**：抽象说法 / 名词化压缩 / 叠着的限定 / 括号补充 / 同义反复，哪一类

只报真站得住的，宁可少报——把「相关但不同的两条规则」误判成重复，会让 PM 删掉真正需要的内容，
代价比漏报大。第 7 类同理：把承重的限定条件当冗余砍掉，比留着啰嗦更糟。"""
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| 1 | {{{{ 待填 }}}} | {{{{ 待填 }}}} | {{{{ 1-6 类之一 }}}} | {{{{ 待填 }}}} | {{{{ 待填 }}}} | {{{{ 收 / 删 / 误报 }}}} |

## 写复杂了清单（第 7 类）

| # | 位置（章节 + 原文片段） | 它其实在说什么 | 改写 | 能砍什么 | triage |
| :--- | :--- | :--- | :--- | :--- | :--- |
| 1 | {{{{ 待填 }}}} | {{{{ 待填 }}}} | {{{{ 待填 }}}} | {{{{ 抽象说法 / 名词化 / 叠限定 / 括号补充 / 同义反复 }}}} | {{{{ 改 / 留 / 误报 }}}} |
