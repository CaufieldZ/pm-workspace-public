...
前置：
- 主 PRD md 存在（single 模式直接透传）
- split 模式需 `{stem}-scenes/` 子文件；缺失不中断，stderr 列清单，
  md 内对应链接行标 `<!-- ⚠ 子文件缺失 -->`
- --help 本身无前置

退出码 / 产物：
- 0 — 无缺失子文件。产物落点：-o 写文件（stderr 报行数），缺省全文走 stdout
- 1 — 有子文件缺失（未传 --check 时仍照常输出）
- 2 — 传 --check 且有缺失子文件（只查引用完整性，不产出 md）
"""
...
_EPILOG = """\
示例：
    python3 .claude/skills/prd/scripts/prd_compose.py projects/<活动线>/deliverables/<季度>/<版本>/prd-<活动>-<版本>.md            # 拼接结果走 stdout
    python3 .claude/skills/prd/scripts/prd_compose.py projects/{项目}/deliverables/prd-{简称}-v{N}.md -o composed.md
    python3 .claude/skills/prd/scripts/prd_compose.py projects/{项目}/deliverables/prd-{简称}-v{N}.md --check   # 只查子文件引用完整性
"""
...
def main() -> int:
    ...
    ap.add_argument("prd", help="主 PRD md 路径（single 透传 / split 展开 scenes/ 子文件）")
