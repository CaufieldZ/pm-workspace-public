...
被 screenshot_for_prd.py 复用（fix_dpi / round_phone_corners 主线路径）。
replace_cell_image* 是 docx 时代遗留（v2 PRD md 化后无人调用）；docx / core.styles 改 lazy
import 避免缺 styles 时整模块崩。
"""
...
def replace_cell_image(cell, img_path: str, width_cm: float = 7.0):
    ...
    """
    from docx.shared import Cm  # noqa: F401  lazy: docx 已是可选依赖（v2 md 化后主路径不用）
...
def replace_cell_image_keep_title(cell, scene_title: str, img_path: str, width_cm: float = 5.0):
    ...
    维护者 反馈（memory `feedback_prd_iter_screenshot_pitfalls.md` 坑 2）：
        「你这些怎么都没写具体是哪个页面？」
    ...
    from core.styles import C, para_run  # noqa: F401  docx 时代遗留依赖
