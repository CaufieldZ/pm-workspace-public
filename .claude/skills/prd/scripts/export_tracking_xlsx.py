...
前置：
- <prd.md> 存在，且内含 10 列埋点表（按表头签名定位：首列「所属页面」+ 第 3 列「事件英文名」，
  不依赖章节号，同签名多张表都收）
- Python 依赖 openpyxl
- --help 本身无前置
...
_r and (_s.path.insert(0, str(_r / "scripts")), __import__("lib.route_log", fromlist=["emit"]).emit("export_tracking_xlsx"))
...
_EPILOG = """\
示例：
    python3 .claude/skills/prd/scripts/export_tracking_xlsx.py projects/<产品线>/prd-<产品线>-baseline.md
    python3 .claude/skills/prd/scripts/export_tracking_xlsx.py projects/<产品线>/tracking-<产品线>.md -o <产品线>-埋点.xlsx
"""
...
def main() -> int:
    ...
    ap.add_argument("-o", "--out", help="输出 xlsx 路径；默认 <prd 基名>-埋点.xlsx 落 PRD 同目录")
    args = ap.parse_args()
