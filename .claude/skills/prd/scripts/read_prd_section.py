...
供主线 / sub-agent 按需读 baseline / PRD 章节，不全量 Read 大文件（> 300 行 baseline 的首选读法）。

行为：
- --toc 输出 `章节号 · 白话名` 列表，章级标 [静态]/[动态]
  （动态 = 变更记录 / 决策 / 排期等演化章；静态 = 描述线上现状）
- 读章（不带点，如 `-s 4`）返回整章内容
- 读节（带点，如 `-s 3.2`）返回该节内容（下一个同级 heading 前为止）
- split 模式下读 5/6/7.x 章节自动去 scenes/ 找对应子文件
- --grep 全文（含 scenes/ 子文件）关键词定位

前置：
- PRD md 存在（baseline / 普通 PRD / delta 均可）
- split 模式自动检测：同级有 `{stem}-scenes/` 子目录即按 split 处理
- --help 本身无前置

退出码：
- 0 — TOC / 章节读取完成（章节号未命中时输出 ⚠ 提示文本，不改变退出码）
- 1 — PRD 路径不存在；--grep 无命中
"""
...
_r and (_s.path.insert(0, str(_r / "scripts")), __import__("lib.route_log", fromlist=["emit"]).emit("read_prd_section"))
...
_EPILOG = """\
示例：
    python3 .claude/skills/prd/scripts/read_prd_section.py projects/<产品线>/prd-<产品线>-baseline.md --toc
    python3 .claude/skills/prd/scripts/read_prd_section.py projects/<产品线>/prd-<产品线>-baseline.md -s 5.1
    python3 .claude/skills/prd/scripts/read_prd_section.py projects/<产品线>/prd-<产品线>-baseline.md -s 4    # 整章
    python3 .claude/skills/prd/scripts/read_prd_section.py projects/<产品线>/prd-<产品线>-baseline.md --grep "<关键词>"
"""
...
def main() -> int:
    ...
    ap.add_argument("prd", help="主 PRD md 路径（baseline / 普通 PRD / delta 均可）")
    g = ap.add_mutually_exclusive_group(required=True)
    g.add_argument("--toc", action="store_true",
                   help="列章节 TOC（章级标 [静态]/[动态]，动态 = 变更 / 决策 / 排期等演化章）")
    g.add_argument("-s", "--section",
                   help="章节号：`4` 读整章 / `5.1` 读小节（split 模式下 5/6/7.x 自动去 scenes/ 子文件）")
    g.add_argument("--grep", help="全文 grep 关键词（含 scenes/ 子文件），输出 `文件:行号: 内容`")
    args = ap.parse_args()
