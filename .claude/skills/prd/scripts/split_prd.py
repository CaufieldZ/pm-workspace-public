...
前置：
- single 模式 PRD md 存在（同级已有 `{stem}-scenes/` 目录则拒绝执行，防覆盖）
- 场景块须为扁平 `### N.x 编号 · 白话名`（嵌套 #### N.x.y 报错停，先拆成并列再跑）
- --help 本身无前置

退出码 / 产物：
- 0 — 拆分完成（或 --dry-run 列出计划）。产物落点：`{stem}-scenes/` 下每场景一个
  子文件 + 原地改写主 md，原文件备份 `{stem}.md.bak`
- 1 — 未找到场景块（无需 split）
- 终止报错 — scenes 目录已存在 / 场景编号不规范 / 嵌套子场景
