...
输入：projects/<项目>/scene-list.md
输出：projects/<项目>/deliverables/scene-list-<项目>.html

参数：
    <项目名>  projects/ 下的项目目录名（必填，位置参数）

前置: 无
退出码：
    0 — 渲染成功
    1 — 未传项目名
    2 — 该项目的 scene-list.md 不存在

示例：
    python3 .claude/skills/scene-list/scripts/render_scene_list.py <项目>
...
_r and (_s.path.insert(0, str(_r / "scripts")), __import__("lib.route_log", fromlist=["emit"]).emit("render_scene_list"))
...
from scripts.lib.html_builder import (  # noqa: E402
    expand_css_imports,
    get_author,
    get_skel_version,
    write_html,
...
# 表格行判定按 GFM 严格口径（行首尾都须 |）；与 check_scene_list._cells 的容错剥尾是有意差异：
# 渲染器按 GFM 解析（不带尾 | 的行本来就不是表格），校验器容错（不 miss 手写行）。真实 scene-list 全合规，两工具编号口径一致。
...
def render_html(project: str, parsed: dict, css: str, gen_time: str, author: str) -> str:
    ...
    skel_meta = (
        f'\n<meta name="x-pm-skel-version" content="{html.escape(skel)}">' if skel else ''
    )

    return f"""<!DOCTYPE html>
<html lang="zh-CN" class="theme-cd-light">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1">{skel_meta}
