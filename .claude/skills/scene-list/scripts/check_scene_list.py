...
参数：
    <scene-list.md>...  待检文件路径，1 个或多个（位置参数）
    --strict            阻断档：有 error 时 exit 2（hook 用）；默认 warn 不阻断

前置: 无
退出码：
    0 — clean / warn（未传 --strict）
    1 — 未传任何 scene-list.md 路径
    2 — 传 --strict 且有 error

示例：
    python3 .claude/skills/scene-list/scripts/check_scene_list.py projects/<项目>/scene-list.md
    python3 .claude/skills/scene-list/scripts/check_scene_list.py projects/<项目>/scene-list.md --strict
"""
