#!/bin/bash
# MRD 评审报告基本卫生自检（完整用法见 usage() / -h）
set -euo pipefail

usage() {
  cat <<'EOF'
MRD 评审报告基本卫生自检：只卡评审报告本身的卫生，不卡 MRD 输入格式（评的是价值不是格式）。

用法:
  bash .claude/skills/mrd-review/scripts/check_mrd.sh <评审报告.md>   # mrd-review Step 6 调用
  bash .claude/skills/mrd-review/scripts/check_mrd.sh -h              # 本帮助

参数:
  <评审报告.md>  评审报告 md 路径（deliverables/mrd-review-*.md）

检查项: 最终立场 / 八维度齐全 / 投票表（≥8 处立场标识）/ 找漏章节 / Sources 章节（≥3 外部 URL）/
评审局限声明 / Go-No-Go 阈值 / 圈数字残留 / 占位符残留 / 空洞建议（WARN）/ CJK 半角标点
（调共享 scripts/check_cjk_punct.py，不在则跳过该节）。
...
trap '_rc=$?; log_event skill "mrd-review" "$([ $_rc -eq 0 -o $_rc -eq 2 ] && echo completed || echo failed)" 2>/dev/null' EXIT
