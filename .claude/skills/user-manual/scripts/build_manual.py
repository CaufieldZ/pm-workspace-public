...
用法:
  ...
  python3 build_manual.py <source.md> --promo               # 营销稿：不生成封面页，缺图常态放行（设计侧后补）
  python3 build_manual.py <source.md> --docx-out <path>     # 自定 docx 落点（默认同名 .docx）

参数:
  <source.md>            手册 / 营销稿源 md 路径（user-manual-*.md / promo-*.md）
  --allow-placeholder    缺图降级 warning（占位阶段用）
  --promo                营销稿模式（复用占位降级逻辑）
...
退出码: 0 = 生成成功；1 = 源 md 不存在 / 缺图（未加降级参数）/ pandoc 转换失败；2 = 参数用法错。
"""
import argparse
import re
import shutil
...
def main():
    ...
    ap.add_argument("source", help="源手册 / 营销稿 md 路径")
