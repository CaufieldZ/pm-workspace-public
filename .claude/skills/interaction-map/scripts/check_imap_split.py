...
EPILOG = """\
示例:
    python3 .claude/skills/interaction-map/scripts/check_imap_split.py <file>.html
    python3 .claude/skills/interaction-map/scripts/check_imap_split.py <file>.html --strict
"""
...
def main():
    ...
    ap.add_argument("files", nargs="+", help="imap-*.html 产物路径，1 个或多个（非 .html 后缀跳过）")
    ap.add_argument("--strict", action="store_true", help="阻断档：未拆分 exit 2（hook 用）；默认 exit 1 仅告警")
    args = ap.parse_args()
