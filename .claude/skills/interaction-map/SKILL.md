...
optional_inputs: [context.md, architecture-diagrams]
...
scripts:
  ...
  check_imap.sh: "Step C 文案讲人话自检 — bash .claude/skills/interaction-map/scripts/check_imap.sh <html>"
...
IMAP 是给 leader / 评审 / 协作方看的产物，正文文案禁出现 PM 内部代号。规则源头见 `pm-workflow.md`「人读产物讲人话」段。

**编号锚点合法位置**（编号即跨 skill 契约，不能砍）：
- `.st > h2`：`{编号} · {业务白话名称}`（例：`B-1 · 我看自己的个人主页`）
- `.phone-label`：`{编号} · {业务白话名称}`（同上格式）
- 元素 `id` 属性 / 锚点 href / 侧导航编号

**正文位置必须业务白话**（禁裸编号 + 决策号）：
- `.flow-note` / `.ann-text` / `.ann-card .card-title` / `.aw .tx`（箭头文案） / `.gd-desc` / `.part-story`
- 反例：`触发 A-1 进入 B 场景`、`见决策 3 处理`、`参考 context 第 7 章 #2`、`见 D5`
- 正例：`从 Feed 点头像进入个人主页`、`订阅功能未来可能付费（本期免费）`、`按业务规则白名单内用户可见`
- `.part-story` 专项：内容必须 = context.md 第 4 章主线分段对应那一句，禁止脱钩自创

**自检脚本**（Step C 必跑）：`bash .claude/skills/interaction-map/scripts/check_imap.sh deliverables/xxx.html`，违反即 fail。
...
3. **PART 故事种子约束**：每个 PART 的 `story` 字段从 context.md 第 4 章「场景叙事顺序」一句话主线分段而来——
   - **叙事型项目** → PART 故事 = 主线分段（context 主线讲完一遍 = 走完所有 PART），三处必须同源（context 第 4 章 → scene-list 顶部 → IMAP `parts[].story`）
   - **规则系统型项目**（纯后台 / 配置型）→ PART 故事写一句 IA 归属或填 `"—"` 显式跳过
   缺字段或随手编造一句话即违规（`_validate_part_stories` 在骨架生成时报错）
4. 将脚本复制到 `projects/{项目名}/scripts/gen_imap_v{N}.py`，修改数据，运行
5. 骨架文件自动生成到 deliverables/
...
> fill 脚本参考：scripts/fill-template.py（仅写 fill 脚本时读取）
...
3. **执行自检**（见下方自检清单）：
   - `bash scripts/check_html.sh deliverables/xxx.html scene-list.md imap` — 结构 / 编号
   - `bash .claude/skills/interaction-map/scripts/check_imap.sh deliverables/xxx.html` — 文案讲人话
   两个脚本必须都 pass 才能交付，违反即返工。
4. **告知用户交付**
...
- 脚本头注释列出本次所有 delta 条目（对应 context.md 第 7 章 D{n} ID，用 `python3 .claude/skills/prd/scripts/active_decisions.py {context.md}` 派生当前 Active）
...
sys.path.insert(0, os.path.join(_WS, '.claude/skills/interaction-map/scripts'))
...
# ═══ 2+. 按 delta 逐条 patch ═══
# 每条对应 context.md 第 7 章一个 D{n} ID，desc 里写「D{n}」便于追溯
# （ID 派生：python3 .claude/skills/prd/scripts/active_decisions.py {context.md}）

# 结构感知方法（推荐）：
# u.replace_annotation('M-A', '3b', '<new text>', 'D20：业务线改多选')
# u.replace_flow_note('M-2', '<new note>', 'D25：配置区重排')
# u.replace_ann_card('M-A', '<old card>', '<new card>', 'D28：标注重写')

# 通用方法（大块替换）：
# u.patch('<old html>', '<new html>', 'D{n}：描述')
...
参考 `scripts/fill-template.py` 的 phone 和 webframe 双示例。
...
```
view .claude/skills/interaction-map/assets/interaction-map-templates.html
```

**按需读取（仅扩展骨架生成器本身时才读）：**
```
view .claude/skills/interaction-map/scripts/gen_imap_skeleton.py
```
...
```python
{
    ...
    "name": "🚀 用户端核心交互",  # 模块名
    "desc": "App 端用户交互流程", # 描述
    "story": "用户从 Feed 看见好友战绩，被吸引点进个人主页",  # 必填 — 用户故事一句话 ≤ 30 字
                                # 来源：context.md 第 4 章场景叙事顺序的主线分段
                                # 三处同源链：context 第 4 章 → scene-list 顶部 → IMAP parts[].story
                                # 规则系统型项目 / 技术骨架 / 数据流类 PART 可填 "—" 显式跳过
                                # 缺字段或与 context 主线脱钩 → 生成报错
