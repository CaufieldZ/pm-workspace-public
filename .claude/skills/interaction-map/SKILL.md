...
**Step B 填充前必读组件速查表**：
```
view .claude/skills/interaction-map/scripts/templates-quickref.md
```
速查表列出所有组件的 class、必填子元素、模板行号。大多数 Scene 凭速查表即可写 fill 函数。

**黄金片段参考（可直接复制修改）：**
```
view .claude/skills/interaction-map/scripts/gold-snippets.md
```
...
```
view .claude/skills/interaction-map/scripts/components-core.md
```

**按需读取业务组件（仅 Scene 包含对应业务组件时才读对应文件，禁止全量读取）：**
- Scene 含交易卡片/K线/持仓/半屏抽屉/跟单 → `view .claude/skills/interaction-map/scripts/biz-trading.md`
- Scene 含 Feed 极简卡/帖子列表/$币种胶囊/交易 CTA → `view .claude/skills/interaction-map/scripts/biz-social.md`
- Scene 含直播间/OBS/连麦/策略卡/主播工作台 → `view .claude/skills/interaction-map/scripts/biz-livestream.md`
