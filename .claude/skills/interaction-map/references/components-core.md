...
### 3.3 多张注释卡片（下沉区自动排布）
一个 scene 多张卡时，直接把它们拼进 scene_fn 返回 dict 的 `anno` 字段即可，无需手写 flex 包裹——`.scene-anno` 区已是 `flex-wrap` 容器，自动横排换行。
```html
<!-- 作为 scene_fn 的 anno 字段：return {'flow': 手机流, 'anno': 下面这段} -->
<div class="ann-card">
  <div class="card-title">卡片 A <span class="ann-tag p0">P0</span></div>
...
本文件为通用组件单一信息源，同时服务 Prototype（通过 `crypto-app-vocabulary.md §A` 路由）。组件用 IMAP 的 CSS 变量名（`var(--dark2)` 等），变量值速查：
...
| var(--dark2) | #161A1E | 卡片 |
| var(--dark3) | #2B3139 | 边框 |
| var(--dark-text) | #EAECEF | 一级文字 |
| var(--dark-text2) | #848E9C | 二级文字 |
| var(--dark-text3) | #5E6673 | 三级 / 禁用 |
| var(--blue) | #2979FF | 主操作 |
| var(--green) | #0ECB81 | 正向 |
| var(--red) | #F6465D | 重要 |
