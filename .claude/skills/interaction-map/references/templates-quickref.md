...
| 标注编号 | `div.anno-n.{色}` | 编号圆点，默认阿拉伯数字 `1 2 3`（须与卡内 `.ann-num` 字符样式一致，驱动 hover 联动）| 109 |
| 下沉区 | `div.scene-anno` | `.flow` 下方整块容器（flex-wrap，自动排布一/多张 ann-card）| 152 |
| 注释卡片 | `div.ann-card` | 下沉到 `.scene-anno` 区的说明卡（经 scene_fn 返回 dict 的 `anno` 字段），含 `.card-title` + `.ann-item`* + `.info-box` | 153-164 |
| 注释条目 | `div.ann-item` > `.ann-num.{色}` + `.ann-text` | 编号圆 + 说明文字 | 155-158 |
| 标签 | `span.ann-tag.{type}` | type: p0/p1/p2（仅优先级；禁 new/chg/del）| 154 |
| 信息框 | `div.info-box.{色}` | 色：blue/green/amber | 163 |
