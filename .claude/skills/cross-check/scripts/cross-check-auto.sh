#!/bin/bash
# cross-check 7 维自动对账（PM 收尾时跑一次,替代部分人工走查）— 完整用法见 usage() / -h
set +e

usage() {
  cat <<'EOF'
cross-check 7 维自动对账：跨产出物一致性 + 单产出物完整性的机械校验，逐维输出 pass/warn/fail。

用法:
  bash .claude/skills/cross-check/scripts/cross-check-auto.sh <产品线/项目>   # 手动跑
  bash .claude/skills/cross-check/scripts/cross-check-auto.sh -h             # 本帮助
  （prd-cross-check-gate hook 在写 deliverables/prd-*.md 后自动调本脚本，warn 不阻断）

参数:
  <产品线/项目>  projects/ 下相对路径：<产品线>（产品线根，取线根 baseline）或 <产品线>/<项目>（campaign 变体，取项目自带 baseline）
...
前置: 在工区根目录跑；<产品线/项目> 下须有 scene-list.md，缺则退出 2。-h/--help 不需要以上前置。

退出码:
  0 = 全过
  1 = 有 ❌ 项（属正常产出）或缺参数
  2 = 项目不存在或缺 scene-list.md
EOF
}
