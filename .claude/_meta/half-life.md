...
<!-- 元数据文件 · 不在主线 Claude 注入路径上 · 维护者 半年 review 用
  归类口径：
...
**review 信号**：一旦所有常用模型都是 1M+，`read_prd_section.py` 按需读不再必要；维护者 直接读全文 baseline 就好。预计 2027 年可以动。
...
**为什么 durable**：业务战略主线。community / livestream / growth 的协同矩阵是 维护者 在 Platform C 做 PM 的战略判断，跟工具无关。
...
- 首次创建：2026-05-01
- 维护：维护者 + 本地 Claude Opus（Opus 出具 review 建议，维护者 决策）
