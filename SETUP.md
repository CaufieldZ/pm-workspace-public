...
| **数据平台**（选装）| 行为分析平台 Sensors（示例：GA / Amplitude / Mixpanel 同理） | 不能查埋点、跑不了 data-report | data-report |
...
工作区不强制 `.env`。如果用了通知集成（Slack / 飞书 / 会议纪要工具 / 企微机器人），把 token 直接 export 到 shell 或写进 `~/.zshrc`：
...
外部数据源相关脚本是**示例集成**——内置实现绑定了特定厂商（会议纪要工具 / Confluence / 行为分析平台 / BI 平台 / Slack），但接口都是 REST，自己换 wiki / 数据平台 / 通知工具时改对应脚本里的 base_url + auth 头就行。

### 8.1 外部数据源（示例集成，按需替换）

| 脚本 | 作用 | 当前绑定 / 替换提示 |
|------|------|---------------------|
| `pull_meeting_notes.py` | 拉会议纪要到 `inputs/meetings/`，剥离转写原文 | 会议纪要工具 → 飞书妙记 / Otter.ai / Notion 同接口模式 |
...
| `slack.py` | 推通知 / 拉历史 | Slack → 飞书 / 会议纪要工具 / 企微机器人替换 webhook |
| `youshu_cli.py` | BI 报告链接生成 / 数据拉取 | BI 平台 → Metabase / Superset / Looker 改 API |
